...
| POST | `/api/cameras` | 카메라 추가 (body: channelSlot — Dashboard Channel Slot 1..MAX_CHANNEL_NUM, 생략 시 자동 배정 / streamingMode? — `'jpeg'\|'webrtc'\|'rtsp-over-websocket'`, webrtcEnabled/rtspOverWebSocketEnabled로 파생) |
...
| POST | `/api/analysis/camera-removed` | streaming 서버 카메라 삭제 시 fire-and-forget 통지 (body: cameraId) — 즉시 정리, 미호출 시 5분 idle-prune 폴백 |
| PUT | `/api/galleries/:id/faces/:faceId` | 얼굴 정보 수정 (name?/galleryId?/photo? — 모든 SERVER_MODE에서 동작, Analysis Server Dashboard 포함) |
| GET | `/api/galleries/match-history` | 얼굴 매칭 이력 조회 (query: limit, cameraId?, galleryType?, from?, to? — cameraName 포함) |
| GET | `/api/ingest-status` | ingest-daemon 연결 상태 (CAPTURE_BACKEND=ingest-daemon 전용) — Streaming Dashboard Channel Group nav 상태 배지 |
