...
- 신규 `/StreamingServer` WebSocket 엔드포인트(`server/src/services/umpStreamingServer.js`)가 RTSP Digest(MD5) 인증(카메라별 저장된 username/password 재사용, 신규 `GET /api/cameras/:id/ump-credentials` — JWT 필수인 유일한 카메라 엔드포인트) 후 WS↔TCP 릴레이를 수행 — RTSP 텍스트·비디오 외 채널은 여전히 그대로 통과시키는 사실상 바이트 릴레이지만, 비디오 트랙의 RTP(H.264 또는 H.265)만은 예외로 키프레임 게이팅을 거침(2026-07-24, §8.16 참고 — 아래 항목).
...
- **클라이언트 번들 로드 순서 주의(2026-07-23 jsdom으로 실측 재현·수정)** — `ump-player.min.js` 최상단 초기화 코드가 `window.log4javascript` 없이는 내부 `new Logger()` 폴백에서 `TypeError: Logger is not a constructor`로 throw(번들 자체 버그, unminified `dist/@melchi45/ump-player.js`도 동일)해 `customElements.define('ump-player', ...)`까지 도달하지 못하고 "Loading RTSP-over-WebSocket player…"에서 무한 대기함. 반드시 **`log4javascript.js` → `crypto-js.js`(RTSP Digest 인증용, 마찬가지로 외부 전역 참조) → `ffmpegAAC.js` → `ump-player.min.js`** 순서로 로드(`RTSPOverWebSocketView.tsx`).
- **`device="nvr"` 필수(camera 아님) + `width`/`height` 속성 필수(2026-07-23, `dist/html`·`dist/docs` 확인)** — `client/node_modules/@melchi45/ump-player/dist/html/*.html`(패키지가 실제로 배포하는 예제)과 JSDoc을 직접 확인한 결과: `channel` 속성은 NVR 모드 전용(`device="camera"`는 `channel` 없이 `profile="H.264"` 같은 스트림 프로파일명만 씀 — channelSlot 라우팅과 안 맞음), `profile_number`는 NVR 모드에서 mandatory. `width`/`height`는 JSDoc에 "optional, but element can not be display"라고 명시되어 있어 없으면 엘리먼트가 아예 렌더링되지 않음 — `RTSPOverWebSocketView.tsx`가 `ResizeObserver`로 타일 크기를 측정해 전달. NVR 모드의 `generateRTSPURL()`은 `LiveChannel/<channel>/media.smp`(채널 번호가 아니라 `LiveChannel/`로 시작)를 만들므로 `umpStreamingServer.js`의 채널 추출 정규식도 경로 맨 앞이 아니라 아무 위치의 첫 숫자 세그먼트를 찾도록 일반화함. 재생은 `autoplay` 속성이 아니라 엘리먼트에 명시적으로 `.play()` 호출.
- **"순수 바이트 릴레이"는 클라이언트→백엔드 방향의 RTSP 요청 URI만큼은 그대로 두면 안 됨(2026-07-23, 실 카메라 라이브 테스트로 발견)** — `<ump-player>`는 `hostname`/`channel`/`profile_number` 속성으로 자기만의 URI(`rtsp://<hostname>/LiveChannel/<channel>/media.smp/profile=<n>`)를 만들어 모든 요청(OPTIONS/DESCRIBE/SETUP/...)에 재사용하는데, 이건 MediaMTX가 실제로 publish받은 경로(`<channelSlot>/media.smp`)와 전혀 다른 문자열이라 그대로 릴레이하면 MediaMTX가 경로를 못 찾고 TCP 연결을 끊어버림(WS 쪽엔 code 1005 No Status Recvd로 나타남 — 인증까지는 성공하고 그 직후 끊기는 게 특징). `umpStreamingServer.js`의 `rewriteRequestUri()`가 요청 라인의 URI만 `rtsp://127.0.0.1:{MEDIAMTX_RTSP_PORT}/{channelSlot}/media.smp`로 치환(헤더/바디는 그대로) — 인증 성공 직후 첫 forward와 이후 매 릴레이 메시지(`relayToBackend()`) 둘 다 적용. 클라이언트→서버 방향은 RTP/RTCP 바이너리가 섞이지 않고 항상 RTSP 텍스트라 이 치환이 항상 안전함.
...
- **채널 슬롯당 RTSP fan-out 세션이 여전히 카메라 fleet 용량을 잠식하는 구조적 문제 — `umpStreamingServer.js`가 이제 ingest-daemon fan-out보다 MediaMTX 직결 경로를 우선**(2026-07-24, [Design_RTSP_Capture_Backend.md](../../../docs/design/Design_RTSP_Capture_Backend.md) §6.38): `WEBRTC_ENGINE=mediamtx`가 활성인 카메라는 MediaMTX RTSP loopback(`:8554`)이 떠 있으면, 뷰어 연결 시 `mediamtxManager.waitForPathReady(cameraId, 400, 100)`으로 먼저 확인해 준비돼 있으면 `rtsp://127.0.0.1:{MEDIAMTX_RTSP_PORT}/{cameraId}`에 직접 붙는다 — 위 두 항목의 ingest-daemon `add_rtsp_publish()` 서브프로세스 경로는 그 loopback이 아직 없을 때만 쓰이는 폴백으로 격하됨. **단, 이 loopback이 실제로 떠 있으려면 `pipelineManager.js`의 `needsMediaMTX`가 그 카메라에 대해 `true`여야 함 — 바로 아래 항목 참고.**
- **RTSP-over-WebSocket 전용(webrtcEnabled=false) 카메라는 위 MediaMTX 직결 경로를 한 번도 못 타고 항상 느린 폴백만 사용하던 결함(2026-07-24, ffmpeg로 실측 재현 — 원본 30fps인데 13.5fps만 수신)**: `needsMediaMTX`가 `camera.webrtcEnabled`만 검사하고 `umpEnabled`는 안 봐서, WebRTC 없이 RTSP-over-WebSocket만 켠 카메라는 `mediamtxManager.addCameraPath(camera.id, ...)`가 호출된 적이 없었음 — 그 결과 `waitForPathReady(camera.id, ...)`가 항상 실패해 매번 ingest-daemon 서브프로세스 폴백(위 두 항목, GIL 경합에 취약)으로 떨어짐. **수정**: `needsMediaMTX`를 `(webrtcEnabled || umpEnabled) && WEBRTC_ENGINE==='mediamtx'`로 확장, `server/src/api/cameras.js`의 `needsRestart`도 `umpEnabled` 변경 시 재시작하도록 동기화(기존엔 "재시작 불필요" 예외였음). 재시작 후 재측정 28~31fps 복구 확인.
- **`RTSPOverWebSocketView.tsx`가 `channel` 속성에 `channelSlot`을 오프셋 없이 그대로 전달해 항상 엉뚱한(channelSlot-1) 카메라로 연결되던 버그(2026-07-24)**: ump-player NVR 모드는 `channel` 속성을 1-based로 받아 내부적으로 `-1`해서 실제 RTSP 채널 번호(`_channel`/`channelId`, `strRtspURL`에 그대로 씀)를 만드는데(`ump-player.js:626-627,1027,5706`), `umpStreamingServer.js`는 그 값을 `camera.channelSlot`과 정확히 일치시켜 카메라를 찾음(`server/src/services/umpStreamingServer.js:294`). `channel={String(camera.channelSlot)}`처럼 오프셋 없이 넘기면 실제로는 `channelSlot-1`인 카메라(혹은 존재하지 않는 채널)로 연결되어, "SPS payload is not available"/"video element do not exist" 에러가 반복되며 재생이 안 됨 — 브라우저 콘솔 로그의 `channel: N` 필드가 의도한 카메라의 channelSlot과 다르면 이 버그부터 의심할 것. **수정**: `channel={String(camera.channelSlot + 1)}`.
- **standalone `ump-player-example.html`은 재생되는데 대시보드(`RTSPOverWebSocketView.tsx`)만 "video element do not exist"로 계속 실패하던 문제(2026-07-24, 같은 심링크 파일을 로드하는데도 동작이 다름)**: `ump-player.js`의 `onUmpError()` switch문에 `0x0304`(SPS not available)/`0x0900`(video element not found) 전용 case가 없어 `default:`로 떨어져 그냥 `error` 이벤트로 dispatch됨 — 벤더 자신도 이 둘을 "RTSP 핸드셰이크 초반 자연 회복 상태"로 취급한다는 뜻. standalone은 이 이벤트를 로그만 남기고 플레이어를 살려두지만, `RTSPOverWebSocketView.tsx`의 `onError`가 매 에러마다 `setError()`로 `<ump-player>` 엘리먼트 자체를 언마운트해서 회복 기회를 없앴음(에러 직후 `document.getElementById(...)`가 `null`인 것으로 확인). **수정**: `error`(치명적 전용)와 `playerNotice`(런타임 비차단 알림) 분리, `<ump-player>`는 계속 마운트해두고 알림은 오버레이 배너로만 표시, `statechange`의 `PLAYING` 신호로 자동 해제. **같은 심링크 파일을 로드하는데 동작이 다르면, 코드 버전 차이가 아니라 "호스팅 컨텍스트가 이벤트/에러를 다르게 처리하는지"부터 의심할 것.**
...
- **`submodules/ump-player` 제거됨(2026-08-04)** — 이 서브모듈(레거시 `app/media/ump/` JS + TypeScript 재작성 `src/player/`)은 `@melchi45/rtsp-over-websocket` npm 패키지로 정식 배포되면서 더 이상 필요 없어져 삭제했습니다. `client/scripts/copyUmpPlayerAssets.js`(서브모듈 자산을 `public/`에 심볼릭 링크하던 postinstall 스크립트)도 함께 삭제. 아래는 그 서브모듈 존재 당시의 이력이며, 현재는 해당하지 않습니다: ~~서브모듈(`submodules/ump-player`, 프로토콜 분석 참고용)과 중첩 서브모듈(`app/media`, `app/external-lib`)은 `.gitmodules`가 private repo HTTPS URL이라 `git submodule update --init`이 "Repository not found"로 실패함~~, ~~`submodules/ump-player` 내부 TypeScript/ESM 마이그레이션 진행 중(2026-07-27~) — 레거시 `app/media/ump/`(87개 JS 파일)를 `src/player/`에 병행 TS 구현으로 점진 포팅~~. 포팅 과정에서 발견된 `updatePlayer()`의 zipPassword 삭제 미적용 레거시 버그(원본이 아닌 복사본에만 적용되던 문제)는 새 패키지 코드에도 남아있는지 별도 확인 필요 — 아직 검증 안 됨.
