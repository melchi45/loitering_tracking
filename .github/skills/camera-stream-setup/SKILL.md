...
## RTSP-over-WebSocket (3번째 재생 경로, 2026-07-23 구현 완료, 2026-08-04 npm 패키지로 전환)

JPEG Capture(Socket.IO)/WebRTC에 이어 **세 번째 카메라 재생 경로**로 `<rtsp-over-websocket>` 웹 컴포넌트(npm `@melchi45/rtsp-over-websocket`, GitHub Packages)를 이용한 RTSP-over-WebSocket을 추가했습니다. 카메라 Add/Edit 화면의 기존 "WebRTC On/Off" 토글이 **Streaming Mode 3-way 선택(JPEG(Default) / WebRTC / RTSP-over-WebSocket)**으로 교체됐습니다.

**설계 상세(필독) → [`docs/design/Design_RTSP_Over_WebSocket.md`](../../../docs/design/Design_RTSP_Over_WebSocket.md) (v3.5) · 운영 가이드 → [`docs/ops/RTSP_Over_WebSocket_Streaming_Setup.md`](../../../docs/ops/RTSP_Over_WebSocket_Streaming_Setup.md) (v1.6)**

**2026-08-04 — 서브모듈에서 npm 패키지로 전환:** `melchi45/rtsp-over-websocket` git 서브모듈(레거시 `<rtsp-over-websocket>` 엘리먼트, ~70개 순차 `<script>` 로드)을 제거하고, 같은 저자가 그 서브모듈의 `src/player/` TypeScript 재작성 결과물을 정식 배포한 `@melchi45/rtsp-over-websocket@1.0.1`(GitHub Packages)로 교체했습니다. `client/src/components/RTSPOverWebSocketView.tsx`는 이제 `import '@melchi45/rtsp-over-websocket'` 사이드이펙트로 `<rtsp-over-websocket>` 커스텀 엘리먼트를 등록합니다. 마이그레이션 전 새 패키지의 실제 소스(`RTSPOverWebSocket.ts`)를 직접 확인해 `observedAttributes`(`hostname/proxy/port/secure/device/channel/profile_number/username/password/width/height` 전부 포함), `channel` 오프셋 규칙(1-based in markup, 0-based on wire — 동일), `device==='nvr'` 분기, 이벤트명(`error`/`statechange`/`waiting`/`statistics`/`meta` 전부 동일)이 레거시 `<rtsp-over-websocket>`와 사실상 1:1 호환됨을 확인한 뒤 진행 — 아래 항목들의 "rtsp-over-websocket.js"/`<rtsp-over-websocket>` 관련 세부 동작(속성 의미·채널 오프셋·이벤트 스위치 분기 등)은 새 패키지에서도 동일하게 적용됩니다. 패키지 자체의 React wrapper(`@melchi45/rtsp-over-websocket`의 `react/Player.tsx`)는 브라우저가 카메라에 직접 SUNAPI 로그인하는 흐름을 전제해 이 프로젝트의 "자격증명은 항상 우리 백엔드에서, 브라우저는 카메라와 직접 통신 안 함" 아키텍처와 안 맞아 **채택하지 않음** — 기존처럼 raw custom element를 직접 다루는 얇은 wrapper 유지. GitHub Packages는 public repo라도 `npm install` 시 항상 인증이 필요 — `client/.npmrc`(gitignored, `client/.npmrc.example` 참고)에 `read:packages` 권한 PAT 필요, CI(`test.yml`/`deploy-pages.yml`)는 `secrets.NPM_GH_PACKAGES_TOKEN` 리포지토리 시크릿으로 구성.

핵심 요점 (아래는 레거시 `<rtsp-over-websocket>` 시절 실측·수정 이력 — 새 `<rtsp-over-websocket>` 패키지에도 동일하게 적용됨, 위 참고):
...
- 신규 `/StreamingServer` WebSocket 엔드포인트(`server/src/services/rtspOverWebSocketServer.js`)가 RTSP Digest(MD5) 인증(카메라별 저장된 username/password 재사용, 신규 `GET /api/cameras/:id/rtsp-over-websocket-credentials` — JWT 필수인 유일한 카메라 엔드포인트) 후 WS↔TCP 릴레이를 수행 — RTSP 텍스트·비디오 외 채널은 여전히 그대로 통과시키는 사실상 바이트 릴레이지만, 비디오 트랙의 RTP(H.264 또는 H.265)만은 예외로 키프레임 게이팅을 거침(2026-07-24, §8.16 참고 — 아래 항목).
- ingest-daemon의 6번째 fan-out은 **on-demand**(WS 브릿지가 뷰어 수를 세어 `POST`/`DELETE /cameras/:id/rtsp-publish` 직접 호출) — 평상시 부하 0.
- 카메라 스키마: 기존 `webrtcEnabled`(boolean)는 그대로 두고 신규 `rtspOverWebSocketEnabled`(boolean)를 추가 — `pipelineManager.js`의 기존 webrtcEnabled 로직에 영향 없음. API 계층(`server/src/api/cameras.js`)의 `streamingModeToFlags()`/`deriveStreamingMode()`가 UI의 `streamingMode`(`'jpeg'|'webrtc'|'rtsp-over-websocket'`)와 이 두 boolean 사이를 변환.
- **클라이언트 번들 로드 순서 주의(2026-07-23 jsdom으로 실측 재현·수정)** — `rtsp-over-websocket.min.js` 최상단 초기화 코드가 `window.log4javascript` 없이는 내부 `new Logger()` 폴백에서 `TypeError: Logger is not a constructor`로 throw(번들 자체 버그, unminified `dist/@melchi45/rtsp-over-websocket.js`도 동일)해 `customElements.define('rtsp-over-websocket', ...)`까지 도달하지 못하고 "Loading RTSP-over-WebSocket player…"에서 무한 대기함. 반드시 **`log4javascript.js` → `crypto-js.js`(RTSP Digest 인증용, 마찬가지로 외부 전역 참조) → `ffmpegAAC.js` → `rtsp-over-websocket.min.js`** 순서로 로드(`RTSPOverWebSocketView.tsx`).
- **`device="nvr"` 필수(camera 아님) + `width`/`height` 속성 필수(2026-07-23, `dist/html`·`dist/docs` 확인)** — `client/node_modules/@melchi45/rtsp-over-websocket/dist/html/*.html`(패키지가 실제로 배포하는 예제)과 JSDoc을 직접 확인한 결과: `channel` 속성은 NVR 모드 전용(`device="camera"`는 `channel` 없이 `profile="H.264"` 같은 스트림 프로파일명만 씀 — channelSlot 라우팅과 안 맞음), `profile_number`는 NVR 모드에서 mandatory. `width`/`height`는 JSDoc에 "optional, but element can not be display"라고 명시되어 있어 없으면 엘리먼트가 아예 렌더링되지 않음 — `RTSPOverWebSocketView.tsx`가 `ResizeObserver`로 타일 크기를 측정해 전달. NVR 모드의 `generateRTSPURL()`은 `LiveChannel/<channel>/media.smp`(채널 번호가 아니라 `LiveChannel/`로 시작)를 만들므로 `rtspOverWebSocketServer.js`의 채널 추출 정규식도 경로 맨 앞이 아니라 아무 위치의 첫 숫자 세그먼트를 찾도록 일반화함. 재생은 `autoplay` 속성이 아니라 엘리먼트에 명시적으로 `.play()` 호출.
- **"순수 바이트 릴레이"는 클라이언트→백엔드 방향의 RTSP 요청 URI만큼은 그대로 두면 안 됨(2026-07-23, 실 카메라 라이브 테스트로 발견)** — `<rtsp-over-websocket>`는 `hostname`/`channel`/`profile_number` 속성으로 자기만의 URI(`rtsp://<hostname>/LiveChannel/<channel>/media.smp/profile=<n>`)를 만들어 모든 요청(OPTIONS/DESCRIBE/SETUP/...)에 재사용하는데, 이건 MediaMTX가 실제로 publish받은 경로(`<channelSlot>/media.smp`)와 전혀 다른 문자열이라 그대로 릴레이하면 MediaMTX가 경로를 못 찾고 TCP 연결을 끊어버림(WS 쪽엔 code 1005 No Status Recvd로 나타남 — 인증까지는 성공하고 그 직후 끊기는 게 특징). `rtspOverWebSocketServer.js`의 `rewriteRequestUri()`가 요청 라인의 URI만 `rtsp://127.0.0.1:{MEDIAMTX_RTSP_PORT}/{channelSlot}/media.smp`로 치환(헤더/바디는 그대로) — 인증 성공 직후 첫 forward와 이후 매 릴레이 메시지(`relayToBackend()`) 둘 다 적용. 클라이언트→서버 방향은 RTP/RTCP 바이너리가 섞이지 않고 항상 RTSP 텍스트라 이 치환이 항상 안전함.
- **on-demand fan-out과 DESCRIBE 사이 레이스(2026-07-23, 실 카메라 라이브 테스트로 발견)** — URI 재작성까지 고쳐도 MediaMTX가 `"no stream is available on path '<channelSlot>/media.smp'"`로 연결을 끊을 수 있음: `_acquireViewer()`가 ingest-daemon의 `POST /rtsp-publish` HTTP 응답만 기다리고, MediaMTX가 그 publish를 실제 "활성"으로 인식하는 시점까지는 안 기다렸기 때문(YouTube 등 다른 MediaMTX 경로에서 `pipelineManager.js`가 이미 겪었던 것과 같은 레이스). `_connectBackend()` 전에 `mediamtxManager.waitForPathReady(\`${channelSlot}/media.smp\`, 8000, 250)`를 추가해 해결 — 기존 코드베이스에 이미 있던 해법을 그대로 재사용.
- **`add_rtsp_publish()`의 동기 `av.open()`이 ingest-daemon 전체를 멈추게 했던 심각한 버그(2026-07-23, 실 카메라 라이브 테스트로 발견)** — 이 메서드가 MediaMTX ANNOUNCE 핸드셰이크(`av.open(..., format="rtsp", ...)`)를 HTTP 요청 핸들러 스레드에서 타임아웃 없이 동기 실행하고 있었음. MediaMTX가 응답 없으면 그 스레드가 무기한 블로킹되고, 그 여파로 **ingest-daemon 워치독의 `/health` 체크까지 같이 실패**(daemon 전체 응답 불능) — `/rtsp-publish` POST/DELETE가 5초 타임아웃으로 실패하는 증상과 함께 나타남. 수정: 실제 `av.open()`을 기존 `_SHARED_STOP_EXECUTOR`(bounded pool)로 넘겨 백그라운드 처리하고 HTTP 핸들러는 즉시 응답. **ingest-daemon에 새 blocking 네트워크 호출을 추가할 땐 항상 HTTP 핸들러 스레드에서 직접 돌리지 말고 이 패턴을 따를 것.**
- **SETUP 요청이 "invalid SETUP path"로 거부되어 OPTIONS→SETUP 무한 재시도 루프(2026-07-23, 실 카메라 라이브 테스트로 발견, 두 차례 수정)** — DESCRIBE까지는 정상 처리되는데 SETUP마다 MediaMTX가 연결을 끊고, 클라이언트가 매번 OPTIONS부터 재시도함. 원인: `rewriteRequestUri()`가 메서드 구분 없이 요청 라인 URI 전체를 고정 베이스 문자열로 통째 치환하고 있었는데, `rtspClient.js`의 SETUP 요청은 `<베이스 URI> + <트랙 식별자>`(예: `/trackID=0`) 형태라 이 접미사까지 지워지면서 SETUP이 DESCRIBE와 동일한 URI가 되어버려 MediaMTX가 거부. **1차 수정**(접두사만 클라이언트 자신의 베이스 `clientBaseUri`로 치환)을 배포한 뒤에도 재발 — 실제로는 클라이언트가 SETUP URI를 자기 베이스가 아니라 **DESCRIBE 응답의 `Content-Base` 헤더**(백엔드→클라이언트 방향이라 재작성 없이 그대로 전달했던, MediaMTX 자신의 베이스 — 이미 올바른 값)로 만들고 있었음. 이 문자열은 `clientBaseUri`로 시작하지 않으니 "알 수 없는 접두사" 폴백(통째 치환)으로 빠져 트랙 접미사가 다시 지워짐 — 고치려던 버그를 폴백 분기가 그대로 재현. **최종 수정**: `rewriteRequestUri(text, clientBaseUri, targetUri)`에 우선순위 3단 분기 — ① 이미 `targetUri`로 시작하면 손대지 않고 그대로 통과, ② `clientBaseUri`로 시작하면 그 접두사만 치환, ③ 둘 다 아니면 통째 치환 폴백. **클라이언트가 세션 내내 재사용하는 "베이스 URI + 메서드별 접미사" 패턴의 프로토콜을 릴레이/재작성할 땐 절대 요청 라인 전체를 통째로 바꾸지 말 것 — 그리고 클라이언트가 참조하는 "베이스"가 하나가 아니라(자기 자신의 원본 URI, 서버가 응답 헤더로 알려준 베이스 등) 여러 개일 수 있다는 점까지 감안해야 함.**
- **`rw_timeout` AVOption이 무시되고 있었음(2026-07-23, 위 SETUP 버그 조사 중 부수 발견)** — publish 쪽 `av.open()`에 추가했던 `rw_timeout`이 매번 `WARNING Some options were not used`로 무시되고 있었음 — FFmpeg `rtsp.c`의 공용 AVOption 클래스가 인식하는 키는 `rw_timeout`이 아니라 이 파일이 읽기 쪽에서 이미 일관되게 써온 `stimeout`. `stimeout`으로 교정(두 곳: io 스레드 재연결 경로, `_open_rtsp_publish_async()`) — 그냥 경고가 아니라 의도한 핸드셰이크 타임아웃 보호 자체가 무효였던 것이므로 방치 금지. **PyAV에 낯선 AVOption을 추가할 땐 항상 서버 로그에서 "Some options were not used" 경고가 없는지 확인할 것.**
- **RTSP 핸드셰이크(OPTIONS~PLAY) 전부 성공, WS로 RTP 데이터도 들어오는데 플레이어가 첫 프레임에서 크래시(`Cannot read properties of null (reading 'byteLength')`, 2026-07-23, 실 카메라 라이브 테스트로 발견)** — 사용자 지적("RTSP-over-WebSocket는 RTSP의 SDP에서 처리")을 계기로 `melchi45/rtsp-over-websocket`의 `MediaSession/VideoSession/h265Session.js`·`Util/h265SPSParser.js` 실제 소스 확인: SDP `a=fmtp`의 `sprop-vps/sprop-sps/sprop-pps`는 `rtspClient.js`가 파싱만 하고 실제로 디코더 상태 시딩에는 전혀 안 씀 — H265 세션은 **오직 인밴드 RTP NAL 유닛**으로만 `vpsPayload`/`spsPayload`/`ppsPayload`를 채우고, 그 전에 마커비트 패킷이 오면 `spsParse(null, "H265")` → `null.byteLength`로 그대로 크래시. 원인은 우리 쪽: `ingest_daemon.py`의 카메라 세션 자체는 최초 접속 시 `idr_seen` 게이트로 첫 키프레임까지 기다리지만, `add_rtsp_publish()`/`add_video_fanout()`으로 **세션이 이미 러닝 중일 때 동적으로 추가되는 fan-out 대상은 이 게이트를 거치지 않고 그 시점에 흘러가던 아무 패킷부터 즉시 받기 시작**함 — H.265는 보통 VPS/SPS/PPS를 IDR과 같은 패킷(GOP당 1회)에 담아 보내므로 GOP 중간에 합류하면 파라미터셋 없는 P-슬라이스만 받다가 크래시. 수정: 동적으로 추가되는 두 진입점(`_open_rtsp_publish_async()`, `add_video_fanout()`의 즉시-오픈 분기) 모두에 `"needsKeyframe": True` 플래그 부여, 패킷 루프의 fan-out 디스패치가 엔트리별로 이 플래그를 보고 다음 키프레임까지 스킵하다가 그때 내림. **RTP/RTSP를 세션 도중 동적으로 새 목적지에 fan-out할 땐, 그 세션의 "최초 접속 시 키프레임 대기" 게이트가 새로 합류하는 엔트리에는 자동으로 적용되지 않는다는 것을 항상 염두에 둘 것 — 엔트리별로 별도 게이트가 필요함.**
- **`_open_rtsp_publish_async()`가 락 없이 공유 `vs`(video template stream)를 건드려 세그폴트(2026-07-24, `dmesg`의 `libavformat.so` 크래시로 확인)** — 연결 재수립 시점과 겹치면 use-after-free. `self._video_fanout_lock`으로 읽기/쓰기 양쪽을 감싸고, 스폰 직전에 `self._rtsp_publish_requested`/`self._video_template_stream` 조건을 락 안에서 재검증하도록 수정. 40회 동시 연결 스트레스 테스트로 재발 없음 확인.
- **RTSP-publish fan-out 처리량이 카메라 원본 30fps에 크게 못 미침(2026-07-24, 실측 ~11fps) — 근본 원인은 스레드가 아니라 프로세스 분리 필요였음**: 스핀카운터 스레드로 자체 검증한 결과, PyAV의 RTSP `mux()`(블로킹 네트워크 쓰기)는 GIL을 놓지 않음 — 같은 프로세스 안에서 아무리 스레드를 분리해도 다른 카메라의 io 스레드가 이 mux() 호출에 막혀 손해를 봄. **수정**: `add_rtsp_publish()`가 이제 같은 프로세스 스레드가 아니라 완전히 별도의 `rtsp_publish_worker.py` **서브프로세스**를 `subprocess.Popen(..., stdin=PIPE)`로 스폰 — 패킷을 길이-프리픽스 프레임으로 stdin에 흘려보내고, 그 프로세스가 독자적인 GIL로 `av.open()+mux()`를 수행. PyAV 인코더 컨텍스트는 `extradata` 직접 설정이 금지(`ValueError`)라 기존 인밴드 파라미터셋 주입 로직에 전적으로 의존.
- **채널 슬롯당 RTSP fan-out 세션이 여전히 카메라 fleet 용량을 잠식하는 구조적 문제 — `rtspOverWebSocketServer.js`가 이제 ingest-daemon fan-out보다 MediaMTX 직결 경로를 우선**(2026-07-24, [Design_RTSP_Capture_Backend.md](../../../docs/design/Design_RTSP_Capture_Backend.md) §6.38): `WEBRTC_ENGINE=mediamtx`가 활성인 카메라는 MediaMTX RTSP loopback(`:8554`)이 떠 있으면, 뷰어 연결 시 `mediamtxManager.waitForPathReady(cameraId, 400, 100)`으로 먼저 확인해 준비돼 있으면 `rtsp://127.0.0.1:{MEDIAMTX_RTSP_PORT}/{cameraId}`에 직접 붙는다 — 위 두 항목의 ingest-daemon `add_rtsp_publish()` 서브프로세스 경로는 그 loopback이 아직 없을 때만 쓰이는 폴백으로 격하됨. **단, 이 loopback이 실제로 떠 있으려면 `pipelineManager.js`의 `needsMediaMTX`가 그 카메라에 대해 `true`여야 함 — 바로 아래 항목 참고.**
- **RTSP-over-WebSocket 전용(webrtcEnabled=false) 카메라는 위 MediaMTX 직결 경로를 한 번도 못 타고 항상 느린 폴백만 사용하던 결함(2026-07-24, ffmpeg로 실측 재현 — 원본 30fps인데 13.5fps만 수신)**: `needsMediaMTX`가 `camera.webrtcEnabled`만 검사하고 `rtspOverWebSocketEnabled`는 안 봐서, WebRTC 없이 RTSP-over-WebSocket만 켠 카메라는 `mediamtxManager.addCameraPath(camera.id, ...)`가 호출된 적이 없었음 — 그 결과 `waitForPathReady(camera.id, ...)`가 항상 실패해 매번 ingest-daemon 서브프로세스 폴백(위 두 항목, GIL 경합에 취약)으로 떨어짐. **수정**: `needsMediaMTX`를 `(webrtcEnabled || rtspOverWebSocketEnabled) && WEBRTC_ENGINE==='mediamtx'`로 확장, `server/src/api/cameras.js`의 `needsRestart`도 `rtspOverWebSocketEnabled` 변경 시 재시작하도록 동기화(기존엔 "재시작 불필요" 예외였음). 재시작 후 재측정 28~31fps 복구 확인.
- **`RTSPOverWebSocketView.tsx`가 `channel` 속성에 `channelSlot`을 오프셋 없이 그대로 전달해 항상 엉뚱한(channelSlot-1) 카메라로 연결되던 버그(2026-07-24)**: rtsp-over-websocket NVR 모드는 `channel` 속성을 1-based로 받아 내부적으로 `-1`해서 실제 RTSP 채널 번호(`_channel`/`channelId`, `strRtspURL`에 그대로 씀)를 만드는데(`rtsp-over-websocket.js:626-627,1027,5706`), `rtspOverWebSocketServer.js`는 그 값을 `camera.channelSlot`과 정확히 일치시켜 카메라를 찾음(`server/src/services/rtspOverWebSocketServer.js:294`). `channel={String(camera.channelSlot)}`처럼 오프셋 없이 넘기면 실제로는 `channelSlot-1`인 카메라(혹은 존재하지 않는 채널)로 연결되어, "SPS payload is not available"/"video element do not exist" 에러가 반복되며 재생이 안 됨 — 브라우저 콘솔 로그의 `channel: N` 필드가 의도한 카메라의 channelSlot과 다르면 이 버그부터 의심할 것. **수정**: `channel={String(camera.channelSlot + 1)}`.
- **standalone `rtsp-over-websocket-example.html`은 재생되는데 대시보드(`RTSPOverWebSocketView.tsx`)만 "video element do not exist"로 계속 실패하던 문제(2026-07-24, 같은 심링크 파일을 로드하는데도 동작이 다름)**: `rtsp-over-websocket.js`의 `onRTSPOverWebSocketError()` switch문에 `0x0304`(SPS not available)/`0x0900`(video element not found) 전용 case가 없어 `default:`로 떨어져 그냥 `error` 이벤트로 dispatch됨 — 벤더 자신도 이 둘을 "RTSP 핸드셰이크 초반 자연 회복 상태"로 취급한다는 뜻. standalone은 이 이벤트를 로그만 남기고 플레이어를 살려두지만, `RTSPOverWebSocketView.tsx`의 `onError`가 매 에러마다 `setError()`로 `<rtsp-over-websocket>` 엘리먼트 자체를 언마운트해서 회복 기회를 없앴음(에러 직후 `document.getElementById(...)`가 `null`인 것으로 확인). **수정**: `error`(치명적 전용)와 `playerNotice`(런타임 비차단 알림) 분리, `<rtsp-over-websocket>`는 계속 마운트해두고 알림은 오버레이 배너로만 표시, `statechange`의 `PLAYING` 신호로 자동 해제. **같은 심링크 파일을 로드하는데 동작이 다르면, 코드 버전 차이가 아니라 "호스팅 컨텍스트가 이벤트/에러를 다르게 처리하는지"부터 의심할 것.**
- **신규 뷰어 접속 시 "SPS payload is not available"(errorCode 772)가 수차례 반복되다 재생되던 노이즈 — WS 브릿지에 키프레임 게이팅 추가로 해결(2026-07-24, §8.16, H265 지원은 2026-07-24 후속 갱신)**: MediaMTX는 이미 라이브 중인 스트림에 신규 RTSP 리더가 붙어도 다음 키프레임을 기다려주지 않고 GOP 중간부터 즉시 데이터를 흘려보낸다 — §8.9의 `needsKeyframe` 게이트는 "카메라→MediaMTX 최초 발행" 구간 전용이라 이 "MediaMTX→신규 WS 뷰어" 구간은 커버하지 않았음. `rtspOverWebSocketServer.js`의 backend→client 릴레이가 RTSP interleaved framing을 인식하도록 확장: DESCRIBE 응답 SDP로 비디오 트랙과 코덱(`H264`/`H265`, rtpmap의 `HEVC` 이름도 인식) 식별 → SETUP 응답의 `Transport: interleaved=X-Y`로 비디오 RTP 채널 확정 → 그 채널의 non-키프레임 슬라이스만 첫 키프레임이 지나갈 때까지 드롭. H.264(RFC 6184, 단일 NAL·FU-A·STAP-A)와 H.265(RFC 7798, 2바이트 NAL 헤더·IRAP 타입 16~23·AP(48)/FU(49) — 번호 체계와 패킷 포맷이 H.264와 전혀 다름)를 `classifyVideoRtpPacket()`이 코덱별로 디스패치해 각각 처리. 판단 불가 시(코덱 H264/H265 둘 다 아님/채널 매핑 실패/4초 타임아웃) 항상 fail-open. 순수 함수 단위 Jest 테스트 30건 추가(`rtspOverWebSocketServer.test.js`). **RTSP-over-WS 릴레이 같은 "거의 투명한 프록시" 계층에 특정 트랙만 예외 처리를 추가할 땐, 그 계층이 원래 하지 않던 프로토콜 파싱(SDP/Transport 상관관계, RTP/NAL)을 최소한으로 새로 들여야 하며, 실패 시 반드시 예전 동작(순수 릴레이)으로 되돌아가도록(fail-open) 설계할 것 — 그리고 코덱마다 NAL 레이아웃이 다르므로, 애초에 H.264 하나만 보고 설계하면 H.265 카메라가 조용히 게이팅 없이 동작(fail-open이라 크래시는 안 나지만 원래 고치려던 노이즈는 그대로 남음)한다는 점을 처음부터 감안할 것.**
- **RTSP-over-WebSocket 재생 화면에 detection bounding box 오버레이가 전혀 안 그려지던 버그(2026-07-30, ingest-daemon→analysis capture image 전송은 정상 확인된 상태에서 보고됨)**: `CameraView.tsx`의 JPEG/WebRTC 분기는 각각 `<img>`/`<video>` 옆에 bbox용 `<canvas ref={canvasRef}>`를 렌더링하지만, RTSP-over-WebSocket 분기(`<RTSPOverWebSocketView>`)에는 이 `<canvas>` 자체가 처음부터 없었음 — `drawOverlay()`를 호출하는 `useEffect`(`detections`/`frameWidth`/`frameHeight` 의존)는 스트리밍 모드와 무관하게 정상 실행되고 있었으나 `if (!canvas || !hasVideo) return;` 가드에서 `canvasRef.current`가 항상 `null`이라 매번 조용히 no-op됐음. 소켓 수신(`useCamera.ts`의 `frame`/`detections` 핸들러)은 `streamingMode`와 무관하게 무조건 동작하므로 데이터는 정상 도착 중이었음 — "그릴 canvas가 없다"는 것만이 문제였음. **수정**: RTSP-over-WebSocket 분기에 다른 두 분기와 동일한 `<canvas>` 추가, 기존 `drawOverlay()`/`frameWidth`/`frameHeight` 스케일링 그대로 재사용(`<rtsp-over-websocket>` 자체의 네이티브 해상도는 불필요 — canvas는 CSS 크기 기준으로 스케일링). 상세: `docs/design/Design_RTSP_Over_WebSocket.md` §8.20.
- `supportSunapi`(SUNAPI 지원 여부)와 무관하게 모든 카메라에 RTSP-over-WebSocket 옵션 노출. YouTube 카메라는 RTSP-over-WebSocket 대상에서 제외(2-way JPEG/WebRTC 그대로).
- **`melchi45/rtsp-over-websocket` 제거됨(2026-08-04)** — 이 서브모듈(레거시 `app/media/` JS + TypeScript 재작성 `src/player/`)은 `@melchi45/rtsp-over-websocket` npm 패키지로 정식 배포되면서 더 이상 필요 없어져 삭제했습니다. `client/scripts/copyRTSPOverWebSocketAssets.js`(서브모듈 자산을 `public/`에 심볼릭 링크하던 postinstall 스크립트)도 함께 삭제. 아래는 그 서브모듈 존재 당시의 이력이며, 현재는 해당하지 않습니다: ~~서브모듈(`melchi45/rtsp-over-websocket`, 프로토콜 분석 참고용)과 중첩 서브모듈(`app/media`, `app/external-lib`)은 `.gitmodules`가 private repo HTTPS URL이라 `git submodule update --init`이 "Repository not found"로 실패함~~, ~~`melchi45/rtsp-over-websocket` 내부 TypeScript/ESM 마이그레이션 진행 중(2026-07-27~) — 레거시 `app/media/`(87개 JS 파일)를 `src/player/`에 병행 TS 구현으로 점진 포팅~~. 포팅 과정에서 발견된 `updatePlayer()`의 zipPassword 삭제 미적용 레거시 버그(원본이 아닌 복사본에만 적용되던 문제)는 새 패키지 코드에도 남아있는지 별도 확인 필요 — 아직 검증 안 됨.
...
| `server/src/services/ingestDaemonControl.js` | ingest-daemon start/stop/restart 공용 로직 — CLI(`ingest:start/stop/restart`)와 Admin API(`/admin/ingest/*`) 공동 사용, 포트 bind 테스트 기반 좀비 데몬 감지 |
| `server/src/scripts/{start,stop,restart}IngestDaemon.js` | 위 서비스 모듈을 감싸는 얇은 CLI 래퍼 |
...
- `GET /stw-cgi/attributes.cgi/attributes` → XML 응답, `<group name="System"><category name="Limit"><attribute name="MaxChannel" type="int" value="N"/></category></group>` 경로에서 `value` 파싱 (벤더 SUNAPI IP Installer 자체의 쿼리 경로 `System/Limit/MaxChannel`과 동일 — 벤더 WiseNet IP Installer 클라이언트 소스로 교차 확인; 해당 서브모듈의 `media/` 디렉토리는 2026-08-10 체크아웃에서 제거됨)
...
**Admin Dashboard Start/Stop/Restart 컨트롤 (Implemented, 2026-07-23)**: 위 모니터링 패널(`IngestDaemonSection.tsx`)에 Administrator가 ingest-daemon 프로세스를 직접 시작/정지/재시작할 수 있는 버튼 3종 추가 완료. `POST /admin/ingest/{start,stop,restart}` (admin 역할 전용, `verifyAccessToken`+`requireRole('admin')`, 매 호출 `AuditService.log()`) — `server/src/services/ingestDaemonControl.js`가 핵심 로직을 담고 있으며, `startIngestDaemon.js`/`stopIngestDaemon.js`/`restartIngestDaemon.js` 세 CLI 스크립트는 이 모듈을 감싸는 얇은 래퍼로 리팩터링되어 CLI와 API가 완전히 동일한 코드 경로를 탄다. 핵심 설계: "실행 중인지"는 `/health` 응답이 아니라 실제 포트 bind 시도(`isPortFree()`)로 판단 — §6.35/§6.36에서 다룬 "좀비 데몬"(프로세스는 살아있는데 HTTP 무응답) 상태에서도 Stop/Restart가 정상 동작한다(이번 기능 범위에서 `stopIngestDaemon.js`의 기존 오탐 버그도 함께 수정됨). `CAPTURE_BACKEND=ingest-daemon`이 유일한 게이팅 조건(Streaming+Combined 모드 모두 사용 가능, analysis 모드는 자동 제외), 응답은 동기(완료까지 대기, restart는 최대 ~11초). Stop/Restart 버튼은 브라우저 `confirm()`으로 캡처 중단을 경고. 상세: [Design_Ingest_Daemon_Control.md](../../../docs/design/Design_Ingest_Daemon_Control.md), [SRS_Ingest_Daemon_Control.md](../../../docs/srs/SRS_Ingest_Daemon_Control.md), [TC_Ingest_Daemon_Control.md](../../../docs/tc/TC_Ingest_Daemon_Control.md), [Ingest_Daemon_Control_Guide.md](../../../docs/ops/Ingest_Daemon_Control_Guide.md). `startServer.js`의 자체 크래시 자동재시작 루프(§6.36)는 로그 relay·자식프로세스 추적 요구사항이 달라 의도적으로 이 공용 모듈에 통합하지 않음(별도 유지).
...
| `services/ingestDaemonControl.js`, `routes/admin.js` (`/admin/ingest/*`), `IngestDaemonSection.tsx` 버튼 | `CLAUDE.md` `/admin` API 표, `docs/design/Design_Ingest_Daemon_Control.md`, `docs/srs/SRS_Ingest_Daemon_Control.md`, `docs/tc/TC_Ingest_Daemon_Control.md`, `docs/ops/Ingest_Daemon_Control_Guide.md`, 이 스킬의 §Admin Dashboard Start/Stop/Restart 컨트롤 항목, `test/tc_runner_cli.js`+`TcRunnerService.js`의 `SUITES` |
