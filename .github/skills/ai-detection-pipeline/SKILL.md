...
**Windows onnxruntime-node CUDA 소스 빌드 — 네이티브 addon 컴파일 + 런타임 DLL 경로 보정 (2026-07-28)**: 공식 prebuilt `onnxruntime-node`는 Windows에서 CUDA를 제공하지 않지만(C++ addon 자체에는 플랫폼 제약 없음, `USE_CUDA` 컴파일 플래그로만 결정), 소스 빌드로 Windows에서도 CUDA 세션 생성+실 추론이 end-to-end로 동작함을 확인(`docs/design/Design_AI_CUDA_Acceleration.md` §12). 핵심 함정 3가지: (1) `js/node`의 `npm install`은 TypeScript만 컴파일하며 실제 네이티브 addon은 `node ./script/build --config=Release --use_cuda --onnxruntime-build-dir=<path>`(cmake-js 내부 호출)로 별도 빌드해야 함 — 파일 존재 여부(`onnxruntime_binding.node`)로 항상 검증. (2) `cmake-js`의 자체 VS 자동 탐지가 최신 설치 버전(VS Insiders 등)을 잘못 선택할 수 있어 `$env:npm_config_msvs_version="2022"`로 강제 지정 필요, `--dll_deps` 인자는 공백 포함 경로(`C:\Program Files\...`)에서 `spawnSync(...,{shell:true})` 토큰화 버그로 깨지므로 사용하지 않고 빌드 후 `Copy-Item`으로 dll 직접 복사. (3) **빌드 성공 ≠ 런타임 동작** — 모든 dll이 물리적으로 존재해도 `InferenceSession.create()`가 `Invalid handle. Cannot load symbol cudnnCreate`로 실패할 수 있음(Windows 기본 DLL 검색 순서가 addon 자신의 폴더를 포함하지 않기 때문) → `server/src/utils/onnxDllPath.js`의 `ensureOnnxCudaDllPath()`를 `server/src/index.js` 최상단(dotenv 로드 직후)에서 1회 호출해 addon dll 폴더를 `PATH` 맨 앞에 추가함으로써 해결. 전체 장애 대응: `docs/ops/ONNX_Runtime_Source_Build_CUDA13.md` 증상 H~K, TC: `docs/tc/TC_AI_CUDA_Acceleration.md` Test Group H(TC-CUDA-H-001/002), Jest 자동화: `test/api/onnx_dll_path.test.js`(주의: 진짜 Jest `describe/test` 스타일이라 `SUITES` 배열에는 등록하지 않음 — `cd server && npx jest --config jest.config.js --rootDir .. --runInBand ../test/api/onnx_dll_path.test.js`로 실행).
