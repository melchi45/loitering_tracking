---
name: project-ump-player-ts-migration-status
description: submodules/ump-player has an active TypeScript/ESM migration (src/player/, legacy app/media/ untouched) — status snapshot and known doc/build issues as of 2026-07-27
metadata:
  type: project
---
...
- Layer 12 (`angularInterface/` — AngularJS 1.x factory/directive glue code) was explicitly deprioritized in the original plan (MRD: "usage unclear, needs team confirmation, lowest priority, out of scope for early conversion") but was completed early (2026-07-27) at the repo owner's explicit request, out of roadmap order. See [[feedback_ump_player_layer_migration_pattern]] for the approach used.
...
**How to apply**: Before trusting any specific phase/FR completion claim in these SDD docs, spot-check against `find submodules/ump-player/src/player -name '*.ts' ! -name '*.test.ts'` and `npm run test:player` rather than the docs' status tables alone.
