...
**설계 상세(필독) → [`docs/design/Design_UMP_Player_RTSP_over_WebSocket.md`](../../../docs/design/Design_UMP_Player_RTSP_over_WebSocket.md) (v3.5) · 운영 가이드 → [`docs/ops/UMP_Player_Streaming_Setup.md`](../../../docs/ops/UMP_Player_Streaming_Setup.md) (v1.6)**
...
- **UMP 전용(webrtcEnabled=false) 카메라는 위 MediaMTX 직결 경로를 한 번도 못 타고 항상 느린 폴백만 사용하던 결함(2026-07-24, ffmpeg로 실측 재현 — 원본 30fps인데 13.5fps만 수신)**: `needsMediaMTX`가 `camera.webrtcEnabled`만 검사하고 `umpEnabled`는 안 봐서, WebRTC 없이 UMP만 켠 카메라는 `mediamtxManager.addCameraPath(camera.id, ...)`가 호출된 적이 없었음 — 그 결과 `waitForPathReady(camera.id, ...)`가 항상 실패해 매번 ingest-daemon 서브프로세스 폴백(위 두 항목, GIL 경합에 취약)으로 떨어짐. **수정**: `needsMediaMTX`를 `(webrtcEnabled || umpEnabled) && WEBRTC_ENGINE==='mediamtx'`로 확장, `server/src/api/cameras.js`의 `needsRestart`도 `umpEnabled` 변경 시 재시작하도록 동기화(기존엔 "재시작 불필요" 예외였음). 재시작 후 재측정 28~31fps 복구 확인.
- **`UmpPlayerView.tsx`가 `channel` 속성에 `channelSlot`을 오프셋 없이 그대로 전달해 항상 엉뚱한(channelSlot-1) 카메라로 연결되던 버그(2026-07-24)**: ump-player NVR 모드는 `channel` 속성을 1-based로 받아 내부적으로 `-1`해서 실제 RTSP 채널 번호(`_channel`/`channelId`, `strRtspURL`에 그대로 씀)를 만드는데(`ump-player.js:626-627,1027,5706`), `umpStreamingServer.js`는 그 값을 `camera.channelSlot`과 정확히 일치시켜 카메라를 찾음(`server/src/services/umpStreamingServer.js:294`). `channel={String(camera.channelSlot)}`처럼 오프셋 없이 넘기면 실제로는 `channelSlot-1`인 카메라(혹은 존재하지 않는 채널)로 연결되어, "SPS payload is not available"/"video element do not exist" 에러가 반복되며 재생이 안 됨 — 브라우저 콘솔 로그의 `channel: N` 필드가 의도한 카메라의 channelSlot과 다르면 이 버그부터 의심할 것. **수정**: `channel={String(camera.channelSlot + 1)}`.
- **standalone `ump-player-example.html`은 재생되는데 대시보드(`UmpPlayerView.tsx`)만 "video element do not exist"로 계속 실패하던 문제(2026-07-24, 같은 심링크 파일을 로드하는데도 동작이 다름)**: `ump-player.js`의 `onUmpError()` switch문에 `0x0304`(SPS not available)/`0x0900`(video element not found) 전용 case가 없어 `default:`로 떨어져 그냥 `error` 이벤트로 dispatch됨 — 벤더 자신도 이 둘을 "RTSP 핸드셰이크 초반 자연 회복 상태"로 취급한다는 뜻. standalone은 이 이벤트를 로그만 남기고 플레이어를 살려두지만, `UmpPlayerView.tsx`의 `onError`가 매 에러마다 `setError()`로 `<ump-player>` 엘리먼트 자체를 언마운트해서 회복 기회를 없앴음(에러 직후 `document.getElementById(...)`가 `null`인 것으로 확인). **수정**: `error`(치명적 전용)와 `playerNotice`(런타임 비차단 알림) 분리, `<ump-player>`는 계속 마운트해두고 알림은 오버레이 배너로만 표시, `statechange`의 `PLAYING` 신호로 자동 해제. **같은 심링크 파일을 로드하는데 동작이 다르면, 코드 버전 차이가 아니라 "호스팅 컨텍스트가 이벤트/에러를 다르게 처리하는지"부터 의심할 것.**
...
- **UMP 재생 화면에 detection bounding box 오버레이가 전혀 안 그려지던 버그(2026-07-30, ingest-daemon→analysis capture image 전송은 정상 확인된 상태에서 보고됨)**: `CameraView.tsx`의 JPEG/WebRTC 분기는 각각 `<img>`/`<video>` 옆에 bbox용 `<canvas ref={canvasRef}>`를 렌더링하지만, UMP 분기(`<UmpPlayerView>`)에는 이 `<canvas>` 자체가 처음부터 없었음 — `drawOverlay()`를 호출하는 `useEffect`(`detections`/`frameWidth`/`frameHeight` 의존)는 스트리밍 모드와 무관하게 정상 실행되고 있었으나 `if (!canvas || !hasVideo) return;` 가드에서 `canvasRef.current`가 항상 `null`이라 매번 조용히 no-op됐음. 소켓 수신(`useCamera.ts`의 `frame`/`detections` 핸들러)은 `streamingMode`와 무관하게 무조건 동작하므로 데이터는 정상 도착 중이었음 — "그릴 canvas가 없다"는 것만이 문제였음. **수정**: UMP 분기에 다른 두 분기와 동일한 `<canvas>` 추가, 기존 `drawOverlay()`/`frameWidth`/`frameHeight` 스케일링 그대로 재사용(`<ump-player>` 자체의 네이티브 해상도는 불필요 — canvas는 CSS 크기 기준으로 스케일링). 상세: `docs/design/Design_UMP_Player_RTSP_over_WebSocket.md` §8.20.
