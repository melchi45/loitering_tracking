...
gst-inspect-1.0 nvdec           # NVIDIA GPU 확인
gst-inspect-1.0 vaapidecodebin  # Intel/AMD GPU 확인
...
### FFmpeg / Pipeline 필수 변수

```env
MAX_PIPELINES=0   # 0 = 무제한. 동시 카메라 수를 제한하려면 양수 입력
```
...
| 관리자 권한 미부여 | `ADMIN_SEED_EMAIL` 중복 선언 | 하나만 유지 (마지막 선언이 덮어씀) |
