...
- 각 항목: `exists`(다운로드 여부), `active`(활성 여부), `downloading`, `converting`, `downloadPercent`, `downloadError`; 비감지기 항목은 `family`, `license`, `manualOnly?`, `docRef?` 추가
- 시리즈 순서: YOLO26 → YOLO12 → YOLO11 → YOLOv8 (감지기 20개 — `SERIES_ORDER`)
- `POST /api/analysis/models/switch { modelId }` → 해당 family의 활성 모델 전환 (family끼리 서로 독립)
- `POST /api/analysis/models/download { modelId }` → 다운로드 시작; YOLO26/YOLO12는 PT→ONNX 자동 변환; `manualOnly` 항목은 409 반환
- 폴링: `setInterval(fetchCatalog, 2000)` — 다운로드/변환 중인 모델이 있을 때만 활성

**YOLO26/YOLO12 다운로드 버튼 레이블:** `↓ PT→ONNX` (서버가 자동 변환)

**Additional Model Families (비감지기 8종 — `EXTENDED_SERIES_ORDER`):**
- 순서: Face Detection → Face Recognition → PPE Detection → Fire & Smoke Detection → Cloth Attribute (PAR) → Human Parsing → Appearance Re-ID
- 컬럼: Model · License · Size · Action (YOLO 테이블과 다른 레이아웃 — `grid-cols-[1fr_auto_auto_auto]`)
...
### Crop 렌더링 규칙 (compact vs. detail)

`cropData`(base64 JPEG, `server/src/services/snapshotService.js`가 640×640/quality 85 기본값으로 생성 — `SNAPSHOT_MAX_DIMENSION`/`SNAPSHOT_JPEG_QUALITY` env로 조정)를 렌더링하는 모든 자리는 아래 두 모드 중 하나를 따릅니다:

| 모드 | `object-fit` | 크기 | 적용 위치 (`DetectionsTimelineInline.tsx`) |
|---|---|---|---|
| Compact marker | `cover` | 고정 (Gantt 필름스트립 `SNAP_W=28 / SNAP_H=34`) | 필름스트립 크롭 아이콘 (L689-725) — 스크러버 용도, 잘림 허용 |
| Detail view | `contain` | `cropWidth`/`cropHeight`로 동적 계산 (CSS `aspectRatio`), `maxHeight` 상한 | 상세 패널 확대 미리보기 `zoomedSnap` (L781-802), crop 썸네일 3열 그리드 (L804-830) |

**규칙:** 사용자가 crop을 확대해서 보는 자리(디테일 패널, "클릭하여 확대")는 반드시 `object-contain`을 사용해 원본 crop이 잘리지 않도록 해야 합니다. 작은 고정 크기 마커(필름스트립처럼 스크러버 아이콘으로만 쓰이는 경우)는 `object-cover`를 유지해도 됩니다. 새 crop 렌더링 위치를 추가할 때는 이 표에 맞춰 분류하고 `PRD_Detection_Snapshot_Search.md` §6.3, `SRS_Fullscreen_Camera_View.md` FR-08과 동기화하세요.
...
| `DetectionsTimelineInline.tsx` (crop `object-fit`/레이아웃 변경) | `SRS_Fullscreen_Camera_View.md` FR-08, `Design_Fullscreen_Camera_View.md` §3.3.4, `PRD_Detection_Snapshot_Search.md` §6.3 |
| `snapshotService.js` (`SNAPSHOT_MAX_DIMENSION`/`SNAPSHOT_JPEG_QUALITY` 기본값 변경) | `PRD/SRS/Design/TC_Detection_Snapshot_Search.md`, `server/.env*` 4종, `ai-detection-pipeline/SKILL.md` |
