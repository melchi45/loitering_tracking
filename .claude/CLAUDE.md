...
│   │   └── logConfigService.js     # 로그 저장 경로/로테이션(분할) 설정 영속화 — `settings` 테이블(row id `logConfig`), 서버 재시작 시 index.js가 자동 복원 + IPC로 startServer.js(실제 파일 writer)에 전파 (Design_Log_Rotation.md)
...
│   │   └── LogRotationPanel.tsx    # 로그 저장 경로/최대 크기/보관 개수 설정 + 현재 활성 파일·아카이브 목록 (Admin Dashboard → System 전용)
...
| POST | `/api/cameras` | 카메라 추가 (body: channelSlot — Dashboard Channel Slot 1..MAX_CHANNEL_NUM, 생략 시 최저 빈 슬롯 자동 배정; maxChannel/supportSunapi/nvrProfiles — SUNAPI/ONVIF NVR 채널 정보; streamingMode? — `'jpeg'\|'webrtc'\|'rtsp-over-websocket'`, UI 편의 필드로 저장 시 `webrtcEnabled`/`rtspOverWebSocketEnabled`(신규) 두 boolean으로 파생됨, 생략 시 `webrtcEnabled` raw 값도 그대로 허용(하위호환) — Design_RTSP_Over_WebSocket.md §7.2) |
...
| GET | `/api/cameras/:id/rtsp-over-websocket-credentials` | RTSP-over-WebSocket RTSP Digest 인증용 저장된 username/password 조회 (JWT 필수 — password를 반환하는 유일한 카메라 엔드포인트) |
| POST | `/api/cameras/:id/rtsp-over-websocket-meta` | RTSP-over-WebSocket 전용 카메라(ingest-daemon 미경유)의 브라우저 측 `<rtsp-over-websocket>` `'meta'` 이벤트(ONVIF MetadataStream XML) relay (JWT 필수, body: `{ xml }`) — `onvifParser.js`의 `parseOnvifXml`/`ingestOnvifEvents`로 ingest-daemon 경로와 동일한 `onvif_events` 저장·dedup·Socket.IO 브로드캐스트 수행, Design_RTSP_Over_WebSocket.md §8.19 |
...
# bgutil-pot-provider(YouTube PO Token provider, 포트 4416)도 기본 포함되어 위 명령으로
# 함께 기동되지만, server/.env의 YTDLP_POT_PROVIDER_ENABLED=true 없이는 서버가 호출하지
# 않음 (opt-in, 미사용 시 무해, Implemented 2026-08-26 — Design_YouTube_RTSP_Ingest.md §12.6)
docker compose up -d bgutil-pot-provider     # bgutil-pot-provider만 개별 기동
