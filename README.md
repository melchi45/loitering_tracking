...
| 21 | Deployment | **Docker Compose** 단일 명령 배포 · **프로덕션 로그**(레벨 필터 · Admin Dashboard 저장 경로/크기 기반 로테이션·개수 기반 자동 삭제 설정, 2026-08-26 — **서버 인스턴스별로 분리 저장**(2026-08-27, 공유 MongoDB 배포에서 서버 간 로그 경로 상호 덮어쓰기 방지) + **기본 로그 경로 Windows 대응**(2026-08-27, `LOG_DIR_WINDOWS`/`LOG_DIR_LINUX`, Windows 기본값 `C:\ProgramData\lts\logs`) — [Design_Log_Rotation.md §3A/§3B](docs/design/Design_Log_Rotation.md)) · `npm run start/stop` 프로세스 관리 · `/admin/system` CPU/메모리/GPU/디스크 메트릭 | Prometheus `/metrics` 미착수 · OpenAPI 문서 미착수 · K8s/Helm 미착수(→ ES-8) | ⚠️ Partial | Dec 1, 2026 |
