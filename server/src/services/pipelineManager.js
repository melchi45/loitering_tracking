...
class PipelineManager {
  ...
  async _doStartCamera(camera) {
    ...
    const requestedRTSPOverWebSocket = !!camera.rtspOverWebSocketEnabled;
    ...
    //   (b) WEBRTC_ENGINE=mediamtx and RTSP-over-WebSocket delivery is requested —
    //       rtspOverWebSocketServer.js prefers this pre-existing camera.id-keyed
    //       MediaMTX pull over ingest-daemon's own on-demand rtsp-publish subprocess
    //       fan-out (Design_RTSP_Capture_Backend.md §6.38) whenever it's available;
    //       without this, RTSP-over-WebSocket-only cameras never get a camera.id path
    //       and always pay the slower fan-out path (2026-07-24, measured ~13.5fps vs
    //       the camera's real 30fps under load), OR
    ...
    const needsMediaMTX = !isYouTube && (
      ((requestedWebRTC || requestedRTSPOverWebSocket) && WEBRTC_ENGINE === 'mediamtx')
