...
| **UMP Player RTSP-over-WebSocket** | [rfp/](rfp/RFP_UMP_Player_RTSP_over_WebSocket.md) | [prd/](prd/PRD_UMP_Player_RTSP_over_WebSocket.md) | [srs/](srs/SRS_UMP_Player_RTSP_over_WebSocket.md) | [design/](design/Design_UMP_Player_RTSP_over_WebSocket.md) | [tc/](tc/TC_UMP_Player_RTSP_over_WebSocket.md) |
| **Ingest Daemon Control** | [rfp/](rfp/RFP_Ingest_Daemon_Control.md) | [prd/](prd/PRD_Ingest_Daemon_Control.md) | [srs/](srs/SRS_Ingest_Daemon_Control.md) | [design/](design/Design_Ingest_Daemon_Control.md) | [tc/](tc/TC_Ingest_Daemon_Control.md) |
...
| [ops/UMP_Player_Streaming_Setup.md](ops/UMP_Player_Streaming_Setup.md) | UMP Player RTSP-over-WebSocket 서브모듈/GitHub Packages 인증 설정, `client/public/` 정적 자산 배치, 카메라별 활성화, 트러블슈팅 | [Design_UMP_Player_RTSP_over_WebSocket](design/Design_UMP_Player_RTSP_over_WebSocket.md) · [SRS_UMP_Player_RTSP_over_WebSocket](srs/SRS_UMP_Player_RTSP_over_WebSocket.md) |
| [ops/Ingest_Daemon_Control_Guide.md](ops/Ingest_Daemon_Control_Guide.md) | Admin Dashboard ingest-daemon Start/Stop/Restart 사용법, 좀비 프로세스 진단, TC 스위트 실행 가이드 | [Design_Ingest_Daemon_Control](design/Design_Ingest_Daemon_Control.md) · [SRS_Ingest_Daemon_Control](srs/SRS_Ingest_Daemon_Control.md) |
...
| 2026-07-10 | **WebRTC/ICE settings relocated to Administrator Dashboard** — for `streaming`/`analysis` server modes, the dashboard's Settings modal (`App.tsx` `SettingsModal`) now shows only the Language selector; STUN/TURN configuration and the ICE connectivity test moved to a new "WebRTC / ICE" Admin Dashboard section (`AdminUsersPage.tsx` `WebRTCSection`, hidden when `SERVER_MODE=analysis`). `combined` mode is unchanged — both surfaces share `useWebRTCConfigStore`. Updated: `PRD/SRS/Design/TC_ICE_Test_UI.md` (v1.1 amendments), `PRD/SRS/TC_STUN_TURN_ICE.md` (cross-reference rows), `SRS/Design/TC_Admin_Dashboard.md` (FR-AD-070~075, §4.3, TC-AD-013~017); 2 new i18n keys added across all 15 locale files |
...
| 2.7 | 2026-07-10 | WebRTC/ICE 설정을 streaming/analysis 모드 Settings 모달에서 Administrator Dashboard로 이전 — ICE_Test_UI/STUN_TURN_ICE/Admin_Dashboard 문서군 v1.1 amendment 반영 |
