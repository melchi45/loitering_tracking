...
This RFP covers the **full AI model catalog** (YOLO detector + face + PPE + fire/smoke + cloth-PAR + the proposed human-parsing/appearance-reid/age-estimation families), **in-browser model download**, and **runtime hot-swap** functionality in the LTS-2026 analysis server (`SERVER_MODE=analysis` or `combined`), surfaced through the Admin Dashboard's AI Models tab.
...
| FR-RFP-MC-002 | The catalog shall include YOLO detector models from YOLO26, YOLO12, YOLO11, and YOLOv8 series in nano/small/medium/large/xlarge sizes (20 detector entries), plus non-detector entries for face detection (SCRFD), face recognition (ArcFace), PPE (mask+helmet), fire & smoke, cloth-PAR, and the proposed human-parsing, appearance-reid, and age-estimation families. |
...
| FR-RFP-MC-013 | For models whose source is bespoke non-YOLO research code with no HTTP-friendly checkpoint host (e.g. PromptPAR — CLIP+fusion architecture, checkpoint on Google Drive), the system shall automate the download/export via a dedicated Python script rather than requiring operators to do it by hand: clone the model-code repository, fetch the checkpoint, run the export on a CUDA GPU, and verify the ONNX output against the PyTorch reference before accepting it. |
