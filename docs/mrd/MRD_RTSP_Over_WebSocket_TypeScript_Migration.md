...
| Pain Point | Impact |
|---|---|
| 테스트 러너 부재 — 회귀를 검증할 자동화된 수단이 전혀 없음 | 카메라 재생 관련 버그 수정 시마다 실제 카메라로 수동 검증해야 하며, 회귀가 프로덕션에서야 발견됨(실제로 `Design_RTSP_Over_WebSocket.md`의 revision history에 기록된 다수의 사후 발견 버그가 이를 방증) |
