...
- **Active 선택은 서버 재시작 후에도 유지됩니다(2026-07-14~)**: Activate/Deactivate 결과는 `settings` 테이블(row id `activeModels`, `DB_TYPE=json`이면 `storage/lts.json`, `DB_TYPE=mongodb`이면 `settings` 컬렉션)에 자동 저장되고, 다음 서버 기동 시 각 family가 기본 모델을 먼저 로드한 직후 저장된 선택으로 다시 전환/언로드됩니다. 예: Cloth Attribute를 OpenPAR로, YOLO Detection Model을 YOLO12n으로 Activate한 뒤 서버를 재시작해도 두 선택 모두 그대로 유지됩니다. 저장된 모델 파일이 이후 삭제되었거나 카탈로그에서 제거된 경우에는 경고 로그만 남기고 해당 family는 기본값을 그대로 사용합니다(기동 실패 없음). 값은 `GET /api/settings/activeModels`로도 직접 조회할 수 있습니다. 상세 설계는 [Design_AI_Model_Catalog.md §11](../design/Design_AI_Model_Catalog.md#11-active-model-persistence-server-restart-survival) 참고. **범위**: `SERVER_MODE=analysis`/`combined`가 공유하는 `analysisApi.js`의 추론 경로만 대상이며, `combined` 모드에서 로컬 카메라를 직접 추론하는 `pipelineManager.js`의 별도 서비스 인스턴스는 대상이 아닙니다(기존에도 `/models/switch`가 그쪽에는 영향을 주지 않던 별개 이슈).
