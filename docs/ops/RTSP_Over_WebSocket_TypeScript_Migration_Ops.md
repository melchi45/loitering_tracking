...
- `dist/player/ump-player.esm.js` — ESM 빌드 (app-react 등 모던 소비자용)
- `dist/player/ump-player.global.js` — IIFE 빌드 (`customElements.define('ump-player', UmpPlayer)` 자동 등록, 레거시 `<script>` 소비자용)
...
**주의(작업 디렉토리)**: `tsc`/`vite`/`vitest` 명령은 반드시 `submodules/ump-player/src/player/` 디렉토리에서 실행해야 한다(해당 위치의 `tsconfig.json`/`vite.config.ts`/`vitest.config.ts`를 참조). 상위 디렉토리에서 실행하면 `Cannot read file 'tsconfig.json'` 에러가 발생하거나(tsc), 명령이 응답 없이 멎는 것처럼 보일 수 있다(vitest가 잘못된 스코프에서 파일을 탐색). 증상이 나타나면 먼저 `pwd`로 현재 위치를 확인한다.
...
```bash
cd submodules/ump-player/src/player
npx vitest run                              # 전체 스위트
npx vitest run custom/UmpPlayer.test.ts     # 특정 파일만
```

**환경**: 기본 환경은 `environment: 'node'`(빠름)이다. DOM/CustomElement가 필요한 파일(`custom/UmpPlayer.test.ts`)은 파일 최상단에 `// @vitest-environment jsdom` docblock을 선언해 파일 단위로 jsdom 환경을 사용한다 — `vitest.config.ts`의 전역 환경을 바꾸지 않는다.
...
| 증상 | 원인 | 조치 |
|---|---|---|
| `tsc: error TS5083: Cannot read file 'tsconfig.json'` | 잘못된 디렉토리에서 실행 (`submodules/ump-player/` 루트 등) | `cd submodules/ump-player/src/player`로 이동 후 재실행 |
