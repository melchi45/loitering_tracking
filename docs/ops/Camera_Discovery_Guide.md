...
WiseNet/Hanwha camera discovery broadcasts a fixed-format request to `255.255.255.255:7701` and listens for unicast replies on port `7711`, per the vendor's own **SUNAPI IP Installer** protocol spec, §3.4 "IP Scan for SUNAPI" (internal reference: `http://55.101.56.209:8080/site/SUNAPI/SUNAPI_ipinstaller.html#_ip_scan_for_sunapi`). The canonical source is `melchi45/wisenet-camera-discovery`'s `src/nodejs/` (`request.js`/`response.js`/`protocol.js`/`udpDiscovery.js`) — but `server/`'s own code (`server/src/utils/udpDiscovery.js`) does **not** read that repo's filesystem path directly (no git submodule since 2026-08-25, see `docs/design/Design_Camera_Discovery.md` §3.1i). It depends exclusively on the `@melchi45/wisenet-udp-discovery` npm package (GitHub Packages), an ordinary `optionalDependencies` entry in `server/package.json`:

```bash
# One-time: copy server/.npmrc.example → server/.npmrc and fill in a GitHub
# PAT with read:packages scope (GitHub Packages always requires auth, even
# for public repos under the same account).
cd server && npm install   # fetches @melchi45/wisenet-udp-discovery (optionalDependencies)
```

**No independent fallback implementation** (2026-07-03; see `docs/design/Design_Camera_Discovery.md` §3.1f/§3.1f-정정): `server/src/utils/udpDiscovery.js` is now a thin re-export of that npm package — **prior to 2026-07-03 this file also contained a full independent fallback implementation** (`UDPDiscoveryFallback`, ~300 lines) for when the git submodule wasn't initialised; before *that* (prior to 2026-07-02) that fallback was itself a broken ONVIF-XML-only stub that silently discovered zero SUNAPI cameras. Both historical implementations are gone. **Resolution is lazy** — merely requiring this file never fails, even if the npm package isn't installed (needed so `SERVER_MODE=analysis`, which never calls `getUDPDiscovery()`, doesn't crash at startup over a dependency it never uses); calling `getUDPDiscovery()` (or accessing one of its re-exported properties) is what actually triggers `require()`, and *that* fails loudly with a clear message if the package genuinely isn't installed — no more silent degrading. If UDP discovery finds nothing at all, confirm the install succeeded: `ls server/node_modules/@melchi45/wisenet-udp-discovery` after `npm install`, and that `server/.npmrc` has a valid `read:packages` token (see above) — without it `npm install` silently skips this `optionalDependencies` entry rather than failing.

**Request opcode note (updated 2026-07-03)**: both implementations now send the spec-documented opcode `6` (`DEF_REQ_SCAN_EX`), built via `UdpRequest` (`@melchi45/wisenet-udp-discovery`'s `sunapi/request.js`) — see `docs/design/Design_Camera_Discovery.md` §3.1e. The previous default, `1` (an earlier/undocumented opcode this fleet also responds to identically), is kept in `udpDiscovery.js`, **commented out, not deleted**, as an immediate rollback: a prior live comparison found sending `nMode=6` draws responses from a much broader, largely unrelated portion of the network in addition to this project's own camera fleet. **If UDP discovery on a given deployment starts surfacing unrelated/unexpected hosts, or another network-hygiene issue traces back to this broadcast, restore the commented-out `nMode=1` packet (or pass `{ nMode: 1 }` to `UdpRequest` in `_sendDiscovery()`) before investigating further** — this is a one-line revert, not a re-implementation. The socket's `'message'` handler emits a `'scanExtConfirmed'` event (via `UdpResponse.parse()`, `response.js`) whenever a genuine `nMode=12` (`DEF_RES_SCAN_EXT`) reply confirms the opcode-6 round-trip is working as documented — useful for confirming discovery is still healthy after this switch on a given network.
...
**`reserved2`/`reserved3` offset note (2026-07-03)**: the vendor spec's §3.2/§3.3 field *tables* omit two 1-byte fields that Annex A's authoritative C structs (`DATAPACKET_(EXT_)IPv4_T`) do include — one right after `chDeviceName` (offset 119), one right after `nHttpMode` (offset 329). Every field from `nHttpPort` onward was shifted one byte and silently wrong until this was found (a real device's `nHttpPort` decoded as `20596` instead of `80`). The struct is 334 bytes total, not 332 — see `docs/design/Design_Camera_Discovery.md` §3.1e/§3.1f and `@melchi45/wisenet-udp-discovery`'s `sunapi/protocol.js`'s `FIELDS`, the single source of truth for the byte layout.
...
| 1.8 | 2026-08-25 | §1a 갱신 — git submodule `submodules/WiseNetChromeIPInstaller` 완전 제거, npm 의존성을 `wisenet-chrome-ip-installer`(github: 참조)에서 `@melchi45/wisenet-udp-discovery`(GitHub Packages, `server/.npmrc` 인증 필요)로 전환. 설치 확인 명령·경로 참조를 새 패키지 기준으로 정정 (`docs/design/Design_Camera_Discovery.md` §3.1i 참고) |
