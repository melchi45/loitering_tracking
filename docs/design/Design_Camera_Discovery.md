...
2. **WiseNet UDP NVR**: After `mapUDPDevice()` (MaxChannel=1 default), `querySunapiMaxChannel()` is called asynchronously (2 s timeout):
   - Queries `GET /stw-cgi/attributes.cgi/attributes` (2026-07-02: corrected — `media.cgi?msubmenu=channellist` and `system.cgi?msubmenu=systeminfo` are not real SUNAPI CGI paths and never returned data) → response is XML; parses the `value` attribute at `<group name="System"><category name="Limit"><attribute name="MaxChannel" .../>` (matches the vendor SUNAPI IP Installer's own `System/Limit/MaxChannel` query path — cross-checked against the vendor WiseNet IP Installer sources; that submodule's `media/` directory was removed from the checkout on 2026-08-10)
...
**`server/src/utils/udpDiscovery.js` fully replaced — npm package only, no inline fallback, no submodule-path read**: The file's `SUBMODULE_CANDIDATES` path-detection logic and the ~300-line `UDPDiscoveryFallback` class (independent socket/parsing implementation, historical record in §3.1c) are both gone. The file is now a thin re-export of `wisenet-chrome-ip-installer` (see below) — **and only that npm package**; it does not read `submodules/WiseNetChromeIPInstaller`'s filesystem path directly at all (an initial draft of this change tried the submodule path first, then the npm package as fallback, matching the git-submodule-vs-npm priority table this section originally had here — that was corrected same-day at explicit user direction: `server/` is meant to depend on the npm package exclusively; the git submodule remains the canonical *source* the npm package is built from, but is not a second runtime install path for `server/` to fall back to).
...
// calls _resolveImpl() on access — see "Lazy resolution" note below.
```

`wisenet-chrome-ip-installer` — `submodules/WiseNetChromeIPInstaller`'s `master` branch (`"github:melchi45/WiseNetChromeIPInstaller#master"`; 2026-08-10까지는 `#nodejs-udp-discovery` 브랜치였고, 그 브랜치가 master에 머지되면서 참조를 전환), added as an `optionalDependencies` entry in `server/package.json`, fetched by ordinary `npm install`. `optionalDependencies` (not `dependencies`) so a failed/skipped install of this one package doesn't abort `npm install` for the rest of the server. Making this installable at all required adding a **root-level `package.json`** to that repo (previously only `nodejs/package.json` existed; npm's git-dependency installer requires a `package.json` at the repo root to recognize it as a package at all — `npm install` failed with `ENOENT` before this was added). That root `package.json` sets `"private": true` (mixes Chrome-extension assets with the Node.js port, not meant for public registry publish), `"main": "nodejs/udpDiscovery.js"`, and `"files": ["nodejs", "sunapi"]` (2026-08-10: SUNAPI 와이어 포맷 모듈 `protocol.js`/`request.js`/`response.js`가 Chrome 확장과 공유되는 `sunapi/`로 이동하면서 `files`에 추가됨) — verified live that `files` is honored even for a git dependency (not just registry `npm publish`), so only `nodejs/` + `sunapi/` (~88KB) installs into `node_modules`, not the Chrome extension's ~1.5MB of unrelated assets (images, `key.pem`, etc.).
