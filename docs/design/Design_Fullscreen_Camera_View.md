...
| **Name 컬럼** | **각 행 좌측 130px 고정 폭 — topicLabel (색상 강조) / sourceToken / [ruleName]** |
| **헤더 카메라명** | **`useCameraStore`로 조회한 `cameraName` 우선, 없으면 `cameraId.slice(0,8)`** |
...
| v1.5 | **Name 컬럼 추가** | ONVIF Timeline 및 Detections Timeline 좌측 Name 컬럼 추가; ONVIF 헤더 cameraName 표시; Detections tick 오프셋 보정 |
...
| 1.5 | 2026-06-26 | §3.2/3.3 Name 컬럼 추가 — ONVIF(130px, sticky Name 헤더, cameraName 표시) + Detections(100px LABEL_W, tick 오프셋 보정, 드래그 너비 보정); §7 변경 이력 업데이트 |
