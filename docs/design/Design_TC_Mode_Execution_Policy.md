...
| WebRTC Engine Modes | FR-WEM-001~033 | mediamtx/mediasoup 엔진 계약·진단 |
...
| Face Search Condition Sync | FR-FSC-001~010 | streaming→analysis 얼굴 등록 위임 + 검색 조건 push/poll 동기화 검증 |
...
```
1. DB Layer
2. Camera (captureOnly) — incl. Dashboard Channel Slot (test/api/channel_slot.test.js)
3. Auth / User
...
| 1.1 | 2026-07-02 | §8.3 등록 위치 규칙에 Dashboard Channel Slot (test/api/channel_slot.test.js, captureOnly) 추가 |
| 1.2 | 2026-07-08 | §5.3에 Face Search Condition Sync (test/api/face_search_condition_sync.test.js, streamingOnly) 추가 |
| 1.3 | 2026-07-23 | §5.1에 WebRTC Engine Modes (test/api/webrtc_engine_modes.test.js, 플래그 없음) 추가 — mediamtx/mediasoup 엔진 계약·진단 검증 |
