...
### 5.4 X-LTS-Meta 헤더 — Non-ASCII 카메라 이름 처리

HTTP/1.1 헤더 값은 Latin-1(ISO-8859-1) 범위에 속하는 바이트만 허용됩니다. 카메라 이름에 한글 등 Non-ASCII 문자가 포함될 경우 Node.js HTTP 모듈이 "Invalid character in header content" 에러를 발생시켜 프레임 전송이 완전히 실패합니다.

**해결책: Base64 인코딩 (analysisClient.js + analysisApi.js)**

```javascript
// analysisClient.js — 송신 측
'X-LTS-Meta': Buffer.from(metaJson).toString('base64'),

// analysisApi.js — 수신 측 (base64 + legacy raw JSON 호환)
const raw     = req.headers['x-lts-meta'] || '{}';
const jsonStr = raw.startsWith('{') ? raw : Buffer.from(raw, 'base64').toString('utf8');
meta          = JSON.parse(jsonStr);
```

- `raw.startsWith('{')` 분기로 기존 legacy(raw JSON) 클라이언트와 하위 호환 유지
- JPEG body는 이미 binary이므로 헤더만 인코딩하면 되며 오버헤드는 수 바이트 수준

### 5.5 Circuit Breaker "reconnected" 로그 스팸 수정

**증상:** `[AnalysisClient][cameraId] reconnected to analysis server`가 초당 수십 회 반복.

**원인:** `_consecutive` 카운터가 전역 공유이므로, 어떤 카메라든 단발성 타임아웃이 발생하면 `_consecutive=1`이 되고, 다음 프레임 성공 시 `_consecutive > 0` 조건이 참이 되어 "reconnected" 로그 출력.

**수정:**
```javascript
// 변경 전 — 단발 실패 후 복구도 "reconnected" 로그 출력
if (this._consecutive > 0) {
  this._consecutive = 0;
  console.log(`[AnalysisClient] reconnected`);
}

// 변경 후 — circuit-open 직전 수준(≥ CIRCUIT_OPEN_THRESHOLD-1 회 연속 실패) 이후만 로그
...
`CIRCUIT_OPEN_THRESHOLD=3` 기준으로, 2회 이상 연속 실패 후 복구 시에만 로그가 출력됩니다. 단발성 네트워크 지터는 조용히 처리됩니다.
