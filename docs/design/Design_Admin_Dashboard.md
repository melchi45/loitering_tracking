...
```typescript
type AdminSection = 'users' | 'ai-models' | 'webrtc' | 'ingest' | 'onvif' | 'audit' | 'system' | 'logs';
```
...
| `ingest` | 🔌 | Ingest Daemon | 카메라별 실시간 연결/Bps·Fps/코덱/Analysis·Streaming 전달 현황 | `serverMode ≠ analysis`(§4.6 참고, `webrtc`와 동일 조건) |
...
### 4.6 Ingest Daemon 섹션 (`IngestDaemonSection`)

**목적:** ingest-daemon 카메라별 실시간 상태 모니터링 — 연결된 채널·RTSP/YouTube URL·수신 Bps/Fps/codec·IP 연결 정보·Capture 실시간 상태·Analysis 서버 송수신량·Streaming 서버 전달량. Socket.IO 실시간 push(`admin:ingest-stats`), 카메라별 카드 UI + 최근 수분 Bps/Fps 시계열 미니 그래프.

**상세 설계 및 구현 결과:** [Design_Ingest_Daemon_Monitoring.md](Design_Ingest_Daemon_Monitoring.md) §8 — Python(`ingest_daemon.py`) 실시간 계측, Node 집계(`ingestStatsAggregator.js`), Socket.IO admin 권한 검증(`verifySocketAdmin`), 클라이언트 UI. 실 카메라 데이터로 종단 간 검증 완료.
