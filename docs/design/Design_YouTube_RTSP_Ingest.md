...
- **yt-dlp 플러그인:** `bgutil-ytdlp-pot-provider` PyPI 패키지를 설치하면 yt-dlp가 자동으로 `bgutil:http-*` provider를 인식한다(`yt_dlp_plugins/extractor/getpot_bgutil*.py`를 yt-dlp가 자체 플러그인 네임스페이스로 스캔·등록 — 패키지 자체에는 `import`로 불러올 top-level 모듈이 없으므로, 설치 여부 검증은 `importlib.metadata.version(...)`로 pip 메타데이터를 확인한다). 설치 로직은 `server/src/scripts/installYtdlpPotPlugin.js`(`npm run install-pot-plugin`) 하나로 일원화했고, `setup-env.linux.sh`/`setup-env.windows.ps1`의 `[4b]` 단계는 이 스크립트를 그대로 호출한다(DRY, 3곳에 중복 구현하지 않음).
