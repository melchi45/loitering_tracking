...
**Feature:** `melchi45/rtsp-over-websocket/app/media` → `melchi45/rtsp-over-websocket/src/player` TypeScript/ESM 재작성
**Version:** 1.3
**Date:** 2026-08-04
...
> **TC-ID 넘버링 노트**: 기존 `TC_RTSP_Over_WebSocket.md`는 `TC-RTSPWS-NNN` 프리픽스를 사용 중이므로(서버 통합/프로토콜 테스트), 이 문서는 겹치지 않도록 `TC-RTSPWSTS-NNN` 프리픽스를 사용한다(라이브러리 내부 TypeScript 마이그레이션 테스트).
>
> **실행 방법**: `cd rtsp-over-websocket/src/player && npx vitest run` — 89개 테스트 파일, 539개 테스트 전부 자동화됨(수동 검증 항목 없음). 이 문서의 "자동화" 표기는 전부 "완료(구현됨)"이다 — 레거시 서버 스위트(`test:tc`, TC-XXX)와 달리 이 마이그레이션은 브라우저 카메라 연동이 필요 없는 라이브러리 레벨 코드이므로 100% 자동화 가능하다.

---

## 레이어별 테스트 케이스

### TC-RTSPWSTS-001: Layer 1 — Exception 클래스군 parity

**SRS:** FR-RTSPWSTS-001
**대상:** `exceptions/{RTSPOverWebSocketError,AuthError,RTCPError,RTSPError,SunapiError}.ts`
...
### TC-RTSPWSTS-002: Layer 2 — 순수 Util 자료구조/파서 parity

**SRS:** FR-RTSPWSTS-002
**대상:** `RTSPOverWebSocketMap`(hashMap), `Queue`, `CircularTypedArrayQueue`, `DigestGenerator`, `H264SPSParser`, `H265SPSParser`, `CommonAudioUtil`, `FishEye3D`, `FishEye3DMulti`
...
### TC-RTSPWSTS-003: Layer 3 — util.js 분리 모듈 + Network 순수 리프 parity
...
### TC-RTSPWSTS-004: Layer 4 — Listen/Decoder, Talk/Encoder parity
...
### TC-RTSPWSTS-005: Layer 5 — MediaSession 코어 + Video/Audio/TextSession parity
...
### TC-RTSPWSTS-006: Layer 6 — Network transport/RTSPoverWebsocket/http 클라이언트 parity+contract
...
### TC-RTSPWSTS-007: Layer 7 — MediaSession 허브(RtpClient, MediaRouter) contract
...
### TC-RTSPWSTS-008: Layer 8 — Listen/Renderer, Talk, Video/Player/* contract
...
### TC-RTSPWSTS-009: Layer 9 — Interface(StreamPlayer/StreamManager), Backup(BackupProvider) contract
...
### TC-RTSPWSTS-010: Layer 10 — Worker 엔트리 6종 + vendor wrap parity/contract
...
### TC-RTSPWSTS-011: Layer 11 — `<rtsp-over-websocket>` 커스텀 엘리먼트(`custom/RTSPOverWebSocket.ts`) contract (jsdom)

**SRS:** FR-RTSPWSTS-011, FR-RTSPWSTS-012, FR-RTSPWSTS-013
**대상:** `custom/RTSPOverWebSocket.ts` (37 tests, `// @vitest-environment jsdom`)
**Steps 및 Expected:**

| # | 항목 | Expected |
|---|---|---|
| 1 | `customElements.get('rtsp-over-websocket')` 등록 확인 | `HTMLElement` 상속 인스턴스 생성 가능 |
...
| 4 | `attributeChangedCallback('channel', ..., '0')` | `RTSPOverWebSocketError` throw (1 미만 채널 거부) — **CEReactions 주의**: `setAttribute()` 경유 호출은 스펙상 예외가 비동기 report되어 동기 propagate 안 됨; 메서드 직접 호출로 검증 |
...
| 13 | `player` 없는 상태에서 `stop()`/`pause()`/`resume()`/`capture()`/`isPlay()` | 전부 `RTSPOverWebSocketError` throw (`isPlay()`는 deprecated로 상태 무관 항상 throw) |
| 14 | `play()` 입력 검증 | username 미설정 시 `AuthError`; playback 모드에서 `startTime` 미설정 시 `RTSPOverWebSocketError` |
| 15 | 커스텀 `addEventListener`/`removeEventListener`/`dispatchEvent` 레지스트리 | 이벤트 detail에 `channelId`/`elementId` 자동 병합; 동일 타입 중복 등록 무시; 해제 후 미호출; null 리스너 등록 시 `RTSPOverWebSocketError` |
...
| 17 | `bestshotfilter` 접근자 | 유효 값 왕복; 음수 값 `RTSPOverWebSocketError` throw |

**자동화:** 완료 (`custom/RTSPOverWebSocket.test.ts`, 37 tests)

---

### TC-RTSPWSTS-012: 전체 빌드 검증

**SRS:** NFR-RTSPWSTS-001, NFR-RTSPWSTS-002
**Steps:** `cd rtsp-over-websocket/src/player && npx tsc -b --force && npx vite build`
**Expected:** `tsc` 0 에러; Vite가 `dist/player/rtsp-over-websocket.{esm,global}.js` 2종 산출, `custom/RTSPOverWebSocket.ts`의 `customElements.define` 사이드이펙트가 두 번들 모두에 포함(루트 `index.ts`가 `./custom`을 re-export)
**자동화:** 완료 (CI 스크립트화는 후속)

---

### TC-RTSPWSTS-013: 전체 회귀 스위트

**SRS:** NFR-RTSPWSTS-003
...
| 버전 | 날짜 | 변경 내용 |
|---|---|---|
| 1.0 | 2026-07-30 | 초기 작성 — Layer 1-11 전체 TC-ID 기록 (TC-RTSPWSTS-001~013), 전부 자동화 완료 상태로 표기 |
| 1.1 | 2026-07-30 | SRS 신규 작성 완료 반영 — 각 TC 항목에 `**SRS:** FR-RTSPWSTS-NNN` 추적 라인 추가, 헤더에 SRS Reference 추가 |
...
| 1.3 | 2026-08-10 | 테스트 케이스 ID `TC-UMPTS-NNN` → `TC-RTSPWSTS-NNN`으로 전면 리네임, 참조하는 `**SRS:** FR-UMPTS-*`/`NFR-UMPTS-*` 라인도 동시 갱신(SRS 문서와 동시 갱신, 테스트 내용 변경 없음); 잔존 레거시 명칭 일괄 정리 — `Ump*` 식별자·`submodules/rtsp-over-websocket` 경로를 `@melchi45/rtsp-over-websocket` npm 패키지의 현행 명칭(`RTSPOverWebSocket*`, 별도 저장소 `melchi45/rtsp-over-websocket`)으로 전면 교체 |
