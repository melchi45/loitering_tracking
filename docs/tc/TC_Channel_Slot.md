...
### TC-CH-C-002a: Newly-added YouTube channel reflects Repeat Playback/Channel Slot/WebRTC without a page reload

**SRS:** FR-CH-030
**Precondition:** Add Camera modal open, YouTube tab, a reachable YouTube URL entered
**Steps:**
1. Check "Repeat Playback", pick a specific (non-default) Channel Slot, and enable "WebRTC Streaming"
2. Submit and wait for the stream to reach `live` (poll completes, modal closes)
3. Without reloading the page, open Edit on the just-created camera and observe the three fields; also check the dashboard grid cell for the picked Channel Slot

**Expected:** Edit modal shows Repeat Playback checked, the picked Channel Slot, and WebRTC Streaming enabled — matching what was submitted; the camera renders in the grid cell for the picked Channel Slot, not the default/next-free one. Regression case for a 2026-07-30 defect where `CameraList.tsx`'s `startYtPoll()` hand-enumerated fields into `addCamera()` and omitted `repeatPlayback`/`channelSlot`, so the Zustand store held `undefined` for those two fields until a full page reload re-fetched `GET /api/cameras` — the server had persisted all three correctly from creation (see `Design_Channel_Slot.md` §5.3c).
...
| 1.15 | 2026-07-30 | TC-CH-C-002a 추가 — YouTube Add 모달에서 생성 직후 Repeat Playback/Channel Slot/WebRTC Streaming이 새로고침 없이도 반영돼야 함 (Design_Channel_Slot.md §5.3c 버그 수정 회귀 케이스), Automated Coverage 문구에 client 테스트 러너 자체가 없다는 점 명시 |
