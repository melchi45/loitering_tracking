...
| **Status** | Active |
| **Date** | 2026-07-22 |
| **Related RFP** | [RFP_UMP_Player_RTSP_over_WebSocket.md](../rfp/RFP_UMP_Player_RTSP_over_WebSocket.md) |
...
카메라 재생 경로에 UMP Player(Hanwha `<ump-player>` 웹 컴포넌트) 기반 RTSP-over-WebSocket을 추가해, JPEG(기본)/WebRTC와 나란히 세 번째 선택지로 제공한다. 핵심은 **카메라 세션을 추가하지 않는 것**이다 — ingest-daemon의 기존 단일 RTSP 세션 뒤에 로컬 RTSP Proxy(MediaMTX 재사용)와 단일 WS 브릿지를 추가해, 몇 명이 UMP로 보든 카메라는 영향받지 않는다.
...
- 카메라별 저장된 RTSP 자격증명을 그대로 재사용해 별도 인증 체계 없이 UMP 재생을 인증한다.
- 뷰어가 실제로 UMP로 볼 때만 리소스를 소비하는 on-demand 모델을 구현한다.
- 카메라 Add/Edit UI의 재생 모드 선택을 JPEG/WebRTC/UMP 3-way로 확장한다.
...
- YouTube/RTMP/HLS 소스에 대한 UMP 지원.
...
| ID | Story |
|---|---|
| US-01 | 운영자로서, 카메라 Edit 화면에서 재생 모드를 "UMP"로 바꿔 Hanwha 네이티브 뷰어와 동일한 방식으로 스트림을 보고 싶다 |
| US-02 | 시스템 관리자로서, 여러 브라우저 탭에서 동시에 UMP로 같은 카메라를 봐도 카메라 쪽 RTSP 세션이 늘어나지 않기를 원한다 |
| US-03 | 시스템 관리자로서, 아무도 UMP로 보지 않는 카메라는 추가 리소스를 소비하지 않기를 원한다(on-demand) |
| US-04 | 운영자로서, UMP 재생 시 카메라 자격증명을 별도로 다시 입력하지 않고 이미 등록된 카메라 정보로 자동 인증되기를 원한다 |
| US-05 | 시스템 관리자로서, ONVIF 전용(비-Hanwha) 카메라에서도 UMP 옵션을 시도해볼 수 있기를 원한다(로컬 프록시가 재서빙하므로 원본 카메라 제약과 무관) |
| US-06 | 시스템 관리자로서, YouTube 카메라 Add/Edit 화면에는 UMP 옵션이 나타나지 않아 혼선이 없기를 원한다 |
...
`pipelineManager.js`가 이미 YouTube 카메라에 대해 yt-dlp/ffmpeg → `rtsp://127.0.0.1:8554/yt/<id>` publish → ingest-daemon이 로컬 경로를 읽는 패턴을 사용 중이다(Design 문서 §4.1). UMP도 동일 패턴을 재사용한다 — 신규 RTSP 서버를 구현하지 않고 기존 MediaMTX 인스턴스에 채널별 경로(`/<channelSlot>`)로 publish한다.
...
몇 개의 브라우저가 UMP로 동일 카메라를 봐도 ingest-daemon의 카메라 세션 수는 항상 1개다. RTSP Proxy(MediaMTX)와 WS 브릿지는 모두 로컬(loopback) 컴포넌트이므로 부하가 늘어도 카메라 안정성에는 영향이 없다(Design 문서 §3).
...
기존 단일 WebRTC On/Off 스위치를 JPEG(Default)/WebRTC/UMP 3버튼 세그먼트 컨트롤로 교체한다. RTSP/YouTube 양쪽 폼에 적용하되 YouTube는 UMP 옵션을 제외한다(Design 문서 §7.2).
...
| 시나리오 | 동작 |
|---|---|
| UMP로 시청 중인 브라우저가 0개가 됨 | ingest-daemon 6번째 fan-out 및 MediaMTX publish가 해제된다(on-demand 해제) |
...
| `supportSunapi=false`인 ONVIF 전용 카메라에서 UMP 선택 | 정상 노출·동작 대상 — 로컬 프록시가 재서빙하므로 원본 카메라의 SUNAPI 지원 여부는 게이팅 조건이 아님 |
