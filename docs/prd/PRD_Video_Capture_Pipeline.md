...
```
server/src/services/
├── captureFactory.js          # CAPTURE_BACKEND 선택 팩토리 (WebRTC OFF 전용)
├── rtspCapture.js             # FFmpeg JPEG 백엔드
├── gstreamerCapture.js        # GStreamer JPEG 백엔드 (nvdec/vaapi 지원)
├── pyavCapture.js             # Python PyAV JPEG 백엔드
├── rtpIngestion.js            # FFmpeg RTP+JPEG 백엔드 (WebRTC ON 전용, 항상 FFmpeg)
├── webrtcGateway.js           # mediasoup Worker/Router/Transport 관리
└── pipelineManager.js         # 전체 파이프라인 오케스트레이터
```

### 2.2 현재 분기 로직 (pipelineManager.js:185–195)

```javascript
const useWebRTC = !!(camera.webrtcEnabled && webrtcGateway.enabled);

if (useWebRTC) {
  // CAPTURE_BACKEND 무시 → 항상 FFmpeg RtpIngestion
  capture = new RtpIngestion(camera.id, rtspUrl, { fps, width: 640 });
  await capture.start();
} else {
  // CAPTURE_BACKEND 환경변수 반영 (ffmpeg|gstreamer|pyav)
  capture = createCapture(camera.id, rtspUrl, { fps, width: 640 });
}
```

**문제점**: `camera.webrtcEnabled=true`이면 CAPTURE_BACKEND는 완전히 무시되어 GStreamer/PyAV 하드웨어 가속을 WebRTC 경로에서 사용할 수 없음.
...
| 단계 | 채택 기술 | 이유 |
|---|---|---|
| Phase 0 | 설정 수정만 | 코드 변경 없이 즉시 효과, 리스크 없음 |
| Phase 1 | **GStreamer RTP Ingestion** | 기존 `gstreamerCapture.js` nvdec/vaapi 코드 재활용, FFmpeg 완전 대체 없이 선택적 활성화 |
| Phase 2 | **MediaMTX Direct WebRTC** | 이미 실행 중, Pion WebRTC로 mediasoup ICE 의존성 제거 |
| Phase 3 | NVIDIA DeepStream (선택) | 30대+ 카메라 엔터프라이즈 확장 시에만 적용 |
...
- `WEBRTC_MODE=mediamtx` 환경변수 시: mediasoup Worker 생성 생략
- MediaMTX REST API (`/v3/config/paths`) 로 카메라 경로 등록/삭제

**파일**: `client/src/hooks/useWebRTC.ts`
- `WEBRTC_MODE=mediamtx` 시: mediasoup-client 대신 `<video>` 에 MediaMTX WebRTC URL 직접 사용
...
| Phase 2 `WEBRTC_MODE=mediamtx` | 기존 mediasoup 경로 동시 유지 (폴백 조건 설정) |
...
**Phase 2 롤백**:
```bash
# WEBRTC_MODE 환경변수 제거 또는 mediasoup으로 설정
