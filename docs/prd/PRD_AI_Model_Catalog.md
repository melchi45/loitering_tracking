...
| US-08 | System administrator | Have my Activate/Deactivate choices survive a server restart | I don't have to re-select every model by hand after every restart/redeploy |
...
### 4.6 Active Model Persistence

- Every successful `/models/switch` or `/models/deactivate` call is persisted to storage (`DB_TYPE=json` → `storage/lts.json`, `DB_TYPE=mongodb` → the `settings` collection) — the same storage backend already used for tracker/analytics settings
- On the next server start, every family's persisted selection (including an explicit Deactivate) is automatically restored before the server starts accepting traffic — the operator does not need to re-select anything after a restart, redeploy, or crash recovery
- If a persisted model was deleted from `server/models/` or removed from the catalog since it was selected, the family falls back to its default and a warning is logged — restart is never blocked by a stale selection
- This applies to every family listed in §3.4 today and to any family added in the future — no per-family engineering work is needed for the persistence itself
...
| AC-12 | After a successful switch/deactivate and a server restart, `GET /api/analysis/models` reflects the same `active` state as immediately before the restart, for every family that was explicitly configured |
...
| 1.5 | 2026-07-14 | Active Model Persistence 기능 추가 — US-08·§4.6 신설: Admin Dashboard Active 선택이 서버 재시작 후 초기화되던 문제 해결, `DB_TYPE`(json/mongodb) 공통 저장, AC-12 추가 |
