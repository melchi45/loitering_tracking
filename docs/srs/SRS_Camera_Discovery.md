...
The endpoints originally specified for FR-CAM-062 (`/stw-cgi/media.cgi?msubmenu=channellist&action=view` and `/stw-cgi/system.cgi?msubmenu=systeminfo&action=view`) do not exist in the real SUNAPI CGI surface and were never validated against an actual device — they always returned `404`/connection errors, meaning `querySunapiMaxChannel()` never successfully resolved a `MaxChannel > 1` in practice regardless of credentials. The correct capability endpoint is `GET /stw-cgi/attributes.cgi/attributes`, confirmed against the vendor's own WiseNet IP Installer client (which queries the identical `System/Limit/MaxChannel` attribute path from the same endpoint — cross-checked against the vendor WiseNet IP Installer sources; the `media/` directory has since been removed from the `submodules/WiseNetChromeIPInstaller` checkout, 2026-08-10). The response is XML (`Content-Type: application/xml`), not JSON — the system **shall** parse it with the `<group>/<category>/<attribute value="...">` structure, not `JSON.parse()`.
...
`server/package.json` **shall** declare `wisenet-chrome-ip-installer` (the `submodules/WiseNetChromeIPInstaller` repository's `master` branch — switched from the since-merged `nodejs-udp-discovery` branch on 2026-08-10) as an `optionalDependencies` entry, fetched by ordinary `npm install` with no separate `git submodule update --init` step. The `WiseNetChromeIPInstaller` repository **shall** carry a root-level `package.json` (`"private": true`, `"main": "nodejs/udpDiscovery.js"`, `"files": ["nodejs", "sunapi"]` — `sunapi` added when the shared wire-format modules moved there) so npm's git-dependency installer recognizes it as an installable package at all.
...
| 1.16 | 2026-08-10 | LTS Engineering Team | FR-CAM-062a 서술 정리 — 교차 확인 근거로 인용하던 `submodules/WiseNetChromeIPInstaller` 내부 벤더 클라이언트 소스 파일 경로를 삭제(해당 서브모듈의 `media/` 디렉토리가 체크아웃에서 제거됨). 검증 사실 자체는 유지 |
| 1.17 | 2026-08-10 | LTS Engineering Team | FR-CAM-087 갱신 — `wisenet-chrome-ip-installer` 의존성 참조를 `#nodejs-udp-discovery`(master에 머지됨)에서 `#master`로 전환; `protocol.js` 단일 소스 경로를 `sunapi/protocol.js`(Chrome 확장과 공유)로 갱신, 루트 `files`를 `["nodejs", "sunapi"]`로 확장, Acceptance 설치 크기 88KB로 갱신. parity 테스트(TC-H-028/029/032/034)와 라이브 탐색으로 재검증 완료 |
