...
본 문서가 다루는 범위:
- 카메라 스키마 `umpEnabled` 필드 및 API 계층 `streamingMode` UI 편의 필드
...
| streamingMode | API 계층에서만 존재하는 UI 편의 필드(`'jpeg'\|'webrtc'\|'ump'`), 저장 시 `{webrtcEnabled, umpEnabled}`로 파생 |
...
### FR-UMP-001 — `umpEnabled` 필드

모든 카메라 레코드는 boolean 필드 `umpEnabled`를 가져야 한다(SHALL). 기본값은 `false`이며, 기존 `webrtcEnabled` 필드와 완전히 독립적으로 존재해야 한다.

### FR-UMP-002 — `streamingMode` API 편의 필드

`POST /api/cameras`, `PUT /api/cameras/:id`는 `streamingMode: 'jpeg'|'webrtc'|'ump'` 필드를 선택적으로 받아야 한다(SHALL). 이 필드는 DB에 그대로 저장되지 않으며, 아래 규칙으로 `{webrtcEnabled, umpEnabled}`로 파생되어야 한다:

| streamingMode | webrtcEnabled | umpEnabled |
|---|---|---|
| `'jpeg'`(기본) | `false` | `false` |
| `'webrtc'` | `true` | `false` |
| `'ump'` | `false` | `true` |
...
### FR-UMP-003 — `streamingMode` 역산

`GET /api/cameras`, `GET /api/cameras/:id` 및 카메라 생성/수정 응답은 저장된 `webrtcEnabled`/`umpEnabled` 값으로부터 `streamingMode`를 역산해 포함해야 한다(SHALL): `umpEnabled === true` → `'ump'`, 그 외 `webrtcEnabled === true` → `'webrtc'`, 그 외 → `'jpeg'`.

**구현 상태: 완료** — `server/src/api/cameras.js`의 `deriveStreamingMode()`.

### FR-UMP-004 — 기존 `webrtcEnabled` 동작 불변
...
### FR-UMP-010 — 6번째 fan-out (로컬 MediaMTX publish)
...
### FR-UMP-011 — 신규 카메라 세션 금지

FR-UMP-010의 fan-out은 카메라에 대한 신규 RTSP 연결/세션을 생성해서는 안 된다(SHALL NOT). ingest-daemon이 이미 열어둔 단일 세션에서 파생되어야 한다.

### FR-UMP-012 — On-demand 시작

FR-UMP-010의 fan-out은 해당 채널에 대한 WS 브릿지 연결이 0개에서 1개로 전환될 때 시작되어야 한다(SHALL) — 기존 `POST /cameras/:id/video-fanout` API를 재사용한다.

### FR-UMP-013 — On-demand 종료

FR-UMP-010의 fan-out은 해당 채널에 대한 WS 브릿지 연결이 1개에서 0개로 전환될 때(마지막 뷰어 종료) 제거되어야 한다(SHALL).

### FR-UMP-014 — MediaMTX 신뢰 모델 불변
...
LTS Node 서버는 `/StreamingServer` 경로에 대해 WebSocket 업그레이드를 수락해야 한다(SHALL). 이 경로는 기존 Socket.IO 서버와 별개의 순수 `ws` 라이브러리 기반 핸들러여야 한다(`<ump-player>`는 Socket.IO 클라이언트가 아님).
...
### FR-UMP-022 — RTSP Digest(MD5) 인증

WS 브릿지는 FR-UMP-021에서 식별된 카메라의 DB에 저장된 `username`/`password`를 이용해 RTSP Digest(MD5) challenge-response 인증을 수행해야 한다(SHALL). 별도의 JWT 또는 신규 인증 체계를 요구해서는 안 된다(SHALL NOT).

### FR-UMP-023 — 인증 실패 처리

RTSP Digest 인증에 실패하면(자격증명 불일치, 또는 카메라에 저장된 자격증명이 없음) WS 브릿지는 내부 MediaMTX 연결 단계(§FR-UMP-024)로 진행해서는 안 된다(SHALL NOT).

### FR-UMP-024 — 내부 RTSP Proxy 연결

인증 성공 후 WS 브릿지는 `rtsp://127.0.0.1:8554/<channelSlot>/media.smp`로 내부 TCP 소켓을 연결해야 한다(SHALL).

### FR-UMP-025 — 순수 바이트 릴레이
...
### FR-UMP-026 — 마지막 연결 종료 감지

WS 브릿지는 특정 채널에 대한 활성 WS 연결 수가 0이 되는 시점을 감지해 FR-UMP-013의 fan-out 종료를 트리거해야 한다(SHALL).
...
| ID | Requirement |
|---|---|
| NFR-UMP-01 | WS 브릿지의 WS↔TCP 릴레이(§FR-UMP-025)는 트랜스코딩을 수행하지 않으며, CPU 오버헤드가 무시 가능한 수준(순수 바이트 복사)이어야 한다 |
...
POST /api/cameras, PUT /api/cameras/:id
  Body 추가 필드: streamingMode?: 'jpeg' | 'webrtc' | 'ump'

GET /api/cameras, GET /api/cameras/:id, POST/PUT 응답
  추가 필드: streamingMode: 'jpeg' | 'webrtc' | 'ump' (파생값, 저장되지 않음)
```
