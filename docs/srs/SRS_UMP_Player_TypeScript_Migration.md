...
### FR-UMPTS-001: Exception 클래스군 (Layer 1)

`exceptions/{UmpError,AuthError,RTCPError,RTSPError,SunapiError}.ts`는 레거시의 옵션 객체 생성 형태(`new XError({message, channelId, elementId, errorCode, place})`)를 지원하고, `name`/`channel`/`element`/`errorCode`/`place`/`message` 필드가 레거시와 동일해야 한다.
**TC:** TC-UMPTS-001

### FR-UMPTS-002: 순수 Util 자료구조/파서 (Layer 2)

`UmpMap`, `Queue`, `CircularTypedArrayQueue`, `DigestGenerator`, `H264SPSParser`, `H265SPSParser`, `CommonAudioUtil`, `FishEye3D`, `FishEye3DMulti`는 결정적 입력에 대해 레거시와 동일한 출력을 산출해야 한다.
**TC:** TC-UMPTS-002

### FR-UMPTS-003: util.js 분리 모듈 + Network 순수 리프 (Layer 3)
...
### FR-UMPTS-004: Listen/Decoder, Talk/Encoder (Layer 4)
...
### FR-UMPTS-005: MediaSession 코어 + Video/Audio/TextSession (Layer 5)
...
### FR-UMPTS-006: Network transport/RTSPoverWebsocket/http 클라이언트 (Layer 6)
...
### FR-UMPTS-010: Worker 엔트리 6종 + vendor wrap (Layer 10)
...
### FR-UMPTS-011: `<ump-player>` 커스텀 엘리먼트 (Layer 11)

`custom/UmpPlayer.ts`는 `customElements.define('ump-player', UmpPlayer)`로 등록되는 `HTMLElement` 서브클래스로서, 레거시 `Custom/ump-player.js`의 다음 공개 계약을 동일하게 노출해야 한다:
...
- 15개 `onUmp*` 콜백 핸들러(`onUmpError`, `onUmpBackup`, `onUmpTimestamp` 등).
...
**TC:** TC-UMPTS-011 (항목 4, 6-11)

### FR-UMPTS-013: 100% 확인된 dead code만 제거 허용

Grep 등으로 호출부가 전무함이 확인된 코드(예: `AviFormatWriter`의 `aviHeader` 필드, `custom/UmpPlayer.ts`의 `_videoElementStyle` 필드)에 한해서만 포팅에서 제외할 수 있다. 확실하지 않은 경우 포팅하고 주석으로 사유를 남긴다.

---

## 5. 비기능 요구사항

### NFR-UMPTS-001: 빌드 산출물 이중화

`vite build`는 ESM(`ump-player.esm.js`)과 IIFE(`ump-player.global.js`) 두 산출물을 생성해야 한다.

### NFR-UMPTS-002: TypeScript strict 모드

`tsconfig.json`은 strict 모드를 사용하며, `tsc -b`는 0 에러로 통과해야 한다. `any` 타입은 애플리케이션 코드에서 금지되며, vendor 경계에서만 `unknown` + 명시적 wrapper로 격리한다.

### NFR-UMPTS-003: 회귀 방지 — 테스트 커버리지

레거시 87개 파일 각각에 대응하는 TS 구현은 최소 1개 이상의 parity 또는 contract 테스트를 가져야 한다.

### NFR-UMPTS-004: 순수 Node 환경 격리
