...
export default function AdminLogPanel({ apiFetch, serverMode }: AdminLogPanelProps) {
  ...
  const filteredLogs = logs.filter(l => visibleLevels.has(l.level));
    ...
    <div className="p-5 flex flex-col gap-4 h-full">
      ...
      <div className="flex flex-wrap gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
        ...
        <div className="flex items-end gap-2">
          <button
            onClick={() => { setAutoScroll(true); setLogs(prev => [...prev]); }}
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto rounded-xl bg-gray-950 border border-gray-800 font-mono text-[11px]"
      >
        {loading && (
        ...
        {!loading && filteredLogs.length === 0 && (
          <div className="flex flex-col items-center justify-center h-24 gap-2 text-gray-600 text-xs">
            ...
            <span>No log entries{source !== 'server' ? ' — log file may not exist yet' : ''}</span>
          </div>
        )}
        {filteredLogs.map((entry, i) => (
          <LogRow key={`${entry.t}-${i}`} entry={entry} />
        ))}
        <div ref={logEndRef} />
      </div>
    </div>
