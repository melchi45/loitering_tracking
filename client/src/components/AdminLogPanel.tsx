...
import { X, Search, Download, Pause, Play, Inbox } from 'lucide-react';
      ...
      <div className="flex-shrink-0 flex flex-col gap-3 px-5 pt-5 pb-3 bg-gray-950">
        ...
        {error && (
          <div className="bg-red-900/40 border border-red-700/50 text-red-300 text-xs rounded-lg px-3 py-2 flex items-center justify-between">
            <span>{error}</span>
            <button onClick={() => setError('')} className="text-red-400 hover:text-red-200 ml-3"><X className="w-3.5 h-3.5" /></button>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
          ...
          <div className="flex items-end gap-2">
            ...
            >
              <Download className="w-3 h-3 inline mr-1 -mt-0.5" /> Auto-scroll
            </button>
            ...
            >
              {paused
                ? <><Play className="w-3 h-3 inline mr-1 -mt-0.5" /> Resume</>
                : <><Pause className="w-3 h-3 inline mr-1 -mt-0.5" /> Pause</>}
            </button>
            ...
            >
              <Download className="w-3 h-3 inline mr-1 -mt-0.5" /> Download
            </button>
        ...
        <div className="flex items-center gap-4 text-xs text-gray-500 pb-1">
          <span>
            ...
            {paused && <span className="ml-2 text-yellow-500 inline-flex items-center gap-0.5"><Pause className="w-2.5 h-2.5" /> Paused</span>}
            {searchQuery && <span className="ml-2 text-blue-400 inline-flex items-center gap-0.5"><Search className="w-2.5 h-2.5" /> filtered</span>}
          </span>
          {source !== 'server' && (
